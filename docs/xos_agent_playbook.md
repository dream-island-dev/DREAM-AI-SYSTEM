...
- **Fix pattern:** `_shared/whapiVelocityGuard.ts` — every non-group Whapi send now classifies as `guest` (full hot/warm/cold per-recipient caps, by inbound-history recency, not `guests` table membership), `staff` (exempt from tier caps, still serialized behind one device-wide `global_min_gap_sec`), or `group` (fully exempt — the risk is 1:1 DM burst patterns, not posting into a group the device already belongs to). `sendWhapiTextGuarded`/`sendWhapiImageGuarded` wrap `_shared/whapiSend.ts` and are wired into every guest-facing call site across `whatsapp-send`, `guest-club-broadcast`, `oritGuestOutbound`, `executiveAssistant` (including a previously-undocumented unthrottled loop in `request_missing_arrival_times`), and the guest paths of `whapi-webhook`/`whatsapp-webhook` — 25 files call `sendWhapiText` repo-wide, only 6 needed the guard, because the other ~19 are staff DMs/groups already low-frequency by nature.
- **Bulk ≠ 1:1 with a delay:** any UI that fans out to >=3 recipients now enqueues into `whapi_outbound_jobs` (jitter-staggered `scheduled_after`, 8–15s) instead of looping sends in the browser — `whapi-bulk-dispatch` (enqueue) + `whapi-queue-drain` (dedicated 1-min pg_cron, deliberately not folded into `whatsapp-cron`'s 15-min guest cadence) drain it server-side, so a closed tab can't leave a batch half-sent. `WaiterPulseDispatchPanel` moved onto this path. `WhatsAppInbox`'s bulk/audience distribution modes hard-block Whapi entirely now (Meta template only) rather than queuing — a manually-typed message to an arbitrary guest filter is exactly the identical-body-to-many pattern this exists to prevent, and Meta templates are already the sanctioned bulk channel.
- **Post-ban state isn't just a flag flip:** `bot_config.whapi_velocity_state.cooldown_until` halves every tier's caps and hard-blocks `cold`-tier sends until cleared — seeded ACTIVE (now + 48h) by migration 274 since this ban was real, not hypothetical. Clear it manually once the device is confirmed healthy again; don't bulk-send anything until then.
- **Schema gotcha:** `notification_log.status` has a hard-coded CHECK constraint (migration 139) — a new status value used by `_shared/automationClaim.ts`'s `finalizeDispatchAttempt` (guest automation's claim-before-send path) must be added to that constraint in the SAME migration that introduces the new status, or the finalize UPDATE fails silently and the row is stuck at `processing` (in_flight) until the 5-minute stale-reclaim window. `rate_limited` was added alongside the guard for exactly this reason.
...
### 2026-07-30 — Premium Day 1 is not ג׳ספר 1 (trailing-digit room match)
- **Symptom:** Housekeeping `1 צק אין` checked in Yossi Abadi (Premium Day 1 day-pass) instead of Nehoray in ג׳ספר 1.
- **Root:** `roomsCanonicallyMatch` compared trailing digits only (`1` === `1`); `resolveSuiteFromEzgoFields("Premium Day 1")` without day-pass guard also returned `ג׳ספר 1` via bare `1` in room name.
- **Fix:** `isDayPassRoomLabel` blocks canonical digit match; day-pass guests excluded in `housekeepingGuestLookup`. Mirror in `suiteRegistry.js` + `guestRoomResolve.ts`.
...
### 2026-07-28 — `guest_suites_channel` must not gate manual Whapi sends
- **One ACC toggle, two different questions:** `guest_suites_channel` answers "where should **cron automation** go?" — Mike set `meta` to prevent bans. Inbox/Override answers "can **staff** send on the Suites device right now?" — that must stay `canStaffSendViaWhapiSuites()` (= not SOS), independent of the automation cohort. Folding both into `isGuestWhapiSuitesEnabled()` blocked manual Whapi the moment automation moved to Meta.
- **Stage-level exceptions belong in routing, not a second global toggle:** `pre_arrival_2d` is low-volume (1 msg/guest) and should stay Whapi even when automation cohort=Meta — pass `stage_key` into `shouldRouteGuestOutboundViaWhapiSuites(guest, trigger)` and `isStageEffectivelyActive({ stage_key, is_active }, guest)` so Stage 1 bypasses Meta-template `is_active=false` without re-enabling Whapi for night_before/morning.

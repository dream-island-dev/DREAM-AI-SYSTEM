...
- orit_cs_agent ➔ OritCustomerServicePanel.js (סוכן שירות לקוחות לאורית — **Graph API read-only** לתיבת M365 שמקבלת Forward מ-`dream-island.co.il`, טעינה דרך `orit-cs-bootstrap` (service-role, עוקף RLS), כפתורי «חברי Outlook 365» + «סנכרן עכשיו», AI סיכום+טיוטות, העתקה ידנית ל-Outlook, דייג'סט בוקר Whapi). אין שליחה מהמערכת. **מובייל (2026-07-16):** master-detail כמו Inbox — רשימה או פרטי פנייה (לא Grid דו-עמודות); כפתור «חזרה לתור» + `history.back` באנדרואיד.

...
> **עדכון אחרון:** 2026-06-25 (session 41 — Culinary UX Split & Strict Enterprise Routing: סצנת `wine.jpg` הפרידה מתוכן מסעדת ערמונים שהיה ממוזג בה בטעות — סצנה חדשה `armonim.jpg` קיבלה את התוכן + 2 כפתורים. סוג CTA שלישי, `OPS_REQUEST`, נוסף לצד `REQUEST`/`LINK` הקיימים — מנתב ללוח התפעול (`tasks`, migration 085, source חדש `portal_room_service`) + Edge Function חדש `guest-portal-ops-request` ששולח התראת WhatsApp ישירה לאדיר (לא לקבוצה — ⚠️ resolution דרך הלוח באפליקציה, לא 👍🏼, ראה §10 session 41 להסבר). ראה §10 session 41 לפירוט מלא.).
>

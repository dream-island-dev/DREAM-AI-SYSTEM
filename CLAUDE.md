...
> **עדכון אחרון:** 2026-07-08 (session 145 — **deployed 04b1a01:** Inbox red dot — שני מסלולי webhook יתומים (`fallback` גנרי "אעביר לצוות הקבלה" + low-confidence FAQ 🤔) וגם בקשות תפעול LLM/Tier-0 כעת מדליקים `human_requested` על שורת ההודעה הנכנסת דרך `patchClaimedInbound` הקיים (אותו מנגנון כמו guest_alerts→`triggerInboxRedAlert`). `GuestAttentionBadge.js`: `fallback_no_match` (🔁). **Deployed** `whatsapp-webhook`+push main; Vercel ~1-2min. ⚠️ לא נבדק חי — דורש הודעת WA אמיתית. שאר session 142/Orit/migration 158 עדיין **מקומי לא committed**.) (session 144 — **deployed c2cb435:** DREAM BOT Inbox — שני באגים אמיתיים תוקנו, לא קשורים זה לזה למרות שהופיעו באותו screenshot. **(1) בועת "💬 תגובה" ריקה במקום תוכן אמיתי** — root cause אומת בקוד (regex testing, לא ניחוש): `inboxReactions.js`'s `REACTION_ADD_WITH_SNIPPET`/`REACTION_ADD_GENERIC` תפסו את האמוג'י כ-`\p{Extended_Pictographic}` בודד (code point יחיד) — אבל תגובות ברירת-מחדל נפוצות ב-WhatsApp הן multi-codepoint: ❤️ (לב + VARIATION SELECTOR-16) ו-👍🏼 (אגודל + Emoji_Modifier לגוון עור) — שתיהן שברו את כל ה-regex המעוגן (anchor), כי אחרי תפיסת התו הראשון בלבד הרגקס ציפה לרווח ומצא את המודיפייר/VS16 במקום. הנפילה הייתה לענף "guest_reaction, לא-פוענח" שהחזיר `{emoji:null, snippet:null}` → בועה ריקה, למרות שהwebhook (`whatsapp-webhook/index.ts` שורה 3495) כתב נכון את האמוג'י+ציטוט המקוריים ל-DB. **תוקן:** `EMOJI_SEQUENCE` — תבנית "emoji grapheme cluster" מלאה (base+modifier/VS16 אופציונלי, שרשור ZWJ לאמוג'י מורכב, או זוג Regional_Indicator לדגלים) — נבדקה מול 9 תרחישי אמוג'י אמיתיים (כולל 👨‍👩‍👧 ZWJ ו-🇮🇱 דגל) + 2 תרחישי טקסט-רגיל-לא-תגובה. גם ה-fallback ל-`intent==="guest_reaction"` בלי התאמת regex שונה מ-stub מזויף ל-`return null` — כך שכל edge-case עתידי (פורמט Meta חדש) נופל להצגת הטקסט הגולמי מה-DB (FAIL VISIBLE §0.3) במקום בועה ריקה. `formatGuestReactionLabel`'s fallback האחרון שונה מ-"תגובה" ל-"תגובת אמוג'י (פרטים חסרים)" (הגנת-עומק, לא אמור להיות reachable יותר דרך parseGuestReactionMessage). **תיקון בצד frontend בלבד — הwebhook לא נגוע ולא צריך redeploy** (הוא כבר כתב את הנתון הנכון; רק ה-JS regex בצד הלקוח לא פענח אותו). 8 בדיקות רגרסיה חדשות. **(2) באנר אדום תקוע "TypeError: Failed to fetch"** — לא קשור לתגובות; אומת מול קוד המקור של `@supabase/postgrest-js` (node_modules) ש-network-level fetch failure (WiFi/sleep-wake) מגיע ל-`err.message` כמילולית `"TypeError: Failed to fetch"` (postgrest-js בונה `` `${fetchError.name}: ${fetchError.message}` `` בעצמו). `fetchAll`/`fetchSince`/`fetchOlder` ב-`WhatsAppInbox.js` כתבו את זה ל-banner גולמי, בלי prefix מסביר, **ומעולם לא ניקו אותו כשה-poll הבא הצליח** — תקלת רשת חד-פעמית וחולפת השאירה באנר אדום קבוע ומטעה, מנותק מהמציאות הנוכחית. **תוקן:** `setNetworkLoadError(prefix, err)`/`clearStaleNetworkError()` — כל אחת מ-3 פונקציות הטעינה כותבת הודעה עם prefix עברי ספציפי לפעולה (למשל "שגיאת רשת בטעינת שיחות") ושומרת אותה ב-ref; בהצלחה הבאה, ה-banner מתנקה **רק אם** הוא עדיין בדיוק הטקסט שהיא כתבה (לא דורס שגיאה אחרת רלוונטית כמו כשל שליחה שעדיין לא נראתה). כפתורי שליחה (חדר-מוכן/תשובה ידנית) קיבלו גם הם prefix קבוע במקום `err.message` גולמי בכשל. `npm run build` נקי, 23/23 suites, 190/190 tests (היו 182). **⚠️ לא נבדק חי בדפדפן** (login דמו נדחה כרגיל בסביבת הפיתוח, ראה מגבלה חוזרת בsessions 47/51/52/127) — Mike צריך לאמת: (א) תגובת ❤️/👍🏼 אמיתית מאורח מציגה את האמוג'י+הציטוט הנכונים בבועה, (ב) באנר שגיאת רשת (אם מופיע) מנקה את עצמו לבד בפעם הבאה שהתקשורת מצליחה, לא נשאר תקוע.) (session 140c — **plan todos complete, מקומי:** `manager-mail-auto-ack`+`runOritAutoAck`; UI snooze/auto-ack toggle/mailbox FAIL VISIBLE; `docs/orit_outlook_connect.md`; 182 tests.) (session 143 — **audit מסע-האורח + 2 תיקונים, חלקית deployed:** audit read-only מלא של שלושת ערוצי היציאה לאורח (cron→send / webhook / manual) מול הקוד החי — 12 שלבי `automation_stages` פעילים, מטריצת סוויטה/יום-כיף, Tier-0 intercepts, ומקורות תוכן. **שני באגים אומתו ותוקנו:** (1) **deployed** — `whatsapp-send` `resolvePipelineTemplateName()`: שיגור ידני `force_channel=meta_template` של `morning_suite`/`morning_welcome` כיבד את `automation_stages.meta_template_name` (שם תבנית חול, migration 102) **לפני** בדיקת שבת → אורח שמגיע בשבת קיבל תבנית עם קבלת חדר 15:00 במקום 18:00. תוקן: מתעלם מה-DB ותמיד מנתב ל-`suite_welcome_morning[_shabbat]` — אותו כלל בדיוק כמו `night_before` באותה פונקציה. המסלול האוטונומי (cron/`morningDispatch`) היה תקין ממילא — הבאג רק בשיגור ידני מ-ACC. נפרס (`functions deploy whatsapp-send`; אומת לפני deploy שהפונקציה לא מייבאת אף `_shared` ששונה מקומית ב-sessions 141b/142 — routingConfig/guestAlertWhapiNotify/automationSchedule אינם ב-imports שלה, אז לא דלף קוד לא-מאושר). (2) **applied ✅** — migration 159: `bot_scripts.checkout_fb_daypass` נזרע (099) עם `trigger_event='ongoing'` בטעות → `BotScriptEditor` נעל את שדה הטקסט («לא רלוונטי — הודעה זו לא שולחת טקסט ישיר») למרות שהטקסט **כן נשלח** לאורחי יום-כיף (ה-dispatch מחפש לפי `script_key`, לא `trigger_event`); תוקן ל-`'checkout_fb'` — label בלבד, אין CHECK constraint (אומת מול 032). ⚠️ `db push` נחסם ע"י מסווג ההרשאות של auto-mode; **וגם אסור blanket push** — `migration list` חשף ש-**157 כבר applied בפרודקשן** (בניגוד לתיעוד session 141b!) וש-**158 (session 142, לא מאושרת) עדיין pending** — `db push` רגיל היה דוחף אותה יחד עם 159. באישור Mike הוחלה 159 ב-push סלקטיבי (158 הוזזה זמנית והוחזרה מיד; רק 159 נדחפה, self-test עבר, `migration list` מאשר 158 עדיין pending-local בלבד). (3) `docs/active_sprint.md` עודכן לשמות התבניות שהקוד החי באמת מנתב אליהם (`night_before_suites[_shabbat]`, `suite_welcome_morning[_shabbat]`, `dream_room_ready1` — התיעוד הישן עוד הציג את שמות טרום-migration-102) + נוסף SQL לאימות `is_active` חי. **ממצאי audit נוספים (לא תוקנו, מתועדים):** `bot_scripts.morning_daypass` אינו יתום כפי שנחשד — בשימוש בשיגור ידני-session ליום-כיף (hardcoded ב-`whatsapp-send`, לא דרך `session_message_script_key`); הדגל המשותף `msg_pre_arrival_sent` (night_before+daypass) בטוח רק בזכות `applies_to` — אין אכיפת DB. commit `ebffe1b` (סלקטיבי — קבצי 141b/142 לא עורבבו, per תקדים session 52). 179 tests, build נקי.) (session 142 — **מקומי, לא deployed:** ניתוב בקשות אורח מלוח תפעול→לוח בקשות+Whapi «בקשות אורחים». `guest-portal-ops-request` כותב `guest_alerts` (`portal_room_service`)+`onGuestAlertInserted` (היה `tasks`+DM לאדיר). `whatsapp-webhook`: `logAdministrativeRequestAlert` במקום `logAdministrativeRequestTask` (ספא/קבלה); הוסרו כפילויות `tasks` מ-financial/stay-change. migration 158 מעבירה היסטוריה פתוחה (`portal_room_service`+`קבלה/בקשות`)→`guest_alerts` וסוגרת tasks כ-`done`. `RequestsBoard` תוויות חדשות. 179 tests, build נקי.) (session 141b — **מקומי, root cause + fix, לא deployed:** "כן,מגיעים!" ולא נשלח Stage 2 — Mike דיווח מסך WhatsApp אמיתי (אורחת עם מספר שהוא גם WhatsApp Business שלה לעסק צדדי "YARDEN"). Audit ישיר מול ה-DB החי (`notification_log` מול `whatsapp_conversations`, לא הנחה) חשף **שני** באגים אמיתיים: **(1) הבאג המערכתי, לא ספציפי לאורחת אחת** — `whatsapp_conversations_intent_check` (CHECK constraint) מעולם לא כלל את הערך `'arrival_confirmed'` — רק את התאום `'confirmation'` (מסלול טקסט מוקלד, נדיר). `handleStage2ArrivalConfirmation()` כותב `intent:"arrival_confirmed"` לכל אישור-הגעה שמגיע **בלחיצת כפתור** (הרוב המכריע בפועל) — כל insert כזה נכשל בשקט מול ה-constraint מאז commit `48e2314` (2026-07-04, session 104b) — `insertGuestOutboundIfNotMuted()` תופס את השגיאה ורק מלוג, לא throw, אז הקוד ממשיך וכותב `guests.msg_stage_2_arrival_sent=true`+`notification_log status="sent"` כאילו הכל תקין. **התוצאה בפועל: ההודעה כן נשלחת לאורח דרך Meta (וריפיקציה: `notification_log` מראה `status="sent"` על 125 שליחות ב-10 הימים האחרונים) — אבל אף פעם לא נרשמת ב-Inbox הפנימי** (רק 9/125 נרשמו — כולן במסלול הטקסט הנדיר, אפס במסלול הכפתור). זו הסיבה שנראה כאילו "Stage 2 לא נשלח" — הצוות פשוט לא רואה את זה ב-`WhatsAppInbox.js`. **migration 157** מרחיבה את ה-constraint (מוסיפה `arrival_confirmed`+`auto_away_message`, לא מצמצמת — בטוח, לא יכול להיכשל מול דאטה קיים). **(2) רעש משני, כן קשור למסך שהוצג** — הטלפון של האורחת הוא גם WhatsApp Business לעסק שלה, עם הודעת-היעדרות אוטומטית ("מחוץ לשעות הפעילות...") שיורה בחזרה בכל פעם שהבוט שולח לה הודעה יזומה (נצפה 5 פעמים בהיסטוריה: 06-27, 06-28×2, 07-04, 07-08) — עד כה נבלעה כטקסט חופשי רגיל, מסווגת `intent="faq"`, וזוכה לתשובת LLM מבלבלת/לא-קשורה (למשל "תודה שעדכנתם! מחכים לכם" בתגובה להודעת-נעדרות). **תוקן:** `isAutoAwayMessage()`/`AUTO_AWAY_MESSAGE_PATTERN` חדשים ב-`_shared/automationSchedule.ts` (זיהוי מבני לפי ניסוח — "מחוץ לשעות הפעילות/העבודה", "שעות (ה)מענה", "out of office" וכו' — לא hardcoded לשם עסק ספציפי), `handleAutoAwayMessage()` ב-webhook (log-only, אותו חוזה בדיוק כמו `handleCourtesyAck` הקיים — שקט, בלי LLM, בלי תשובה) מחווט Tier-0 גם pre-burst וגם post-burst, מיד אחרי בדיקת הנימוס-הקצר הקיימת. נבדק ידנית (Node script) מול 13 מקרים (4 התאמות אמיתיות + 9 הודעות אורח גנואיניות שאסור שיתאימו, כולל "מה שעות הפעילות של הספא?" כ-false-positive מכוון-למניעה) — כולם עברו. `WhatsAppInbox.js`'s `INTENT_LABELS` קיבל תוויות עברית/אנגלית לשני האינטנטים החדשים. `npm run build` נקי, `npm test` 22/22 suites 179/179 tests. ⚠️ **לא deployed** — ממתין לאישור Mike (`db push` migration 157 + `functions deploy whatsapp-webhook --no-verify-jwt`); לא הוצע/בוצע backfill רטרואקטיבי ל-~116 שורות Stage-2 היסטוריות שחסרות ב-Inbox (נשלחו בהצלחה, רק לא נרשמו) — זו החלטה נפרדת שדורשת אישור מפורש (bulk insert לטבלת audit חיה), לא הוכללה אוטומטית במיגרציה.) (session 141 — **audit+2 secrets set:** Orit CS Agent — «חברי את Outlook» לא עבד. אומת ישירות (לא הנחה): migrations 155/156 pushed + 5 ה-Edge Functions `ACTIVE` (`supabase functions list`) — זו לא בעיית deploy. `supabase secrets list` הראה ש-`MICROSOFT_CLIENT_ID`/`MICROSOFT_CLIENT_SECRET`/`MICROSOFT_TENANT_ID` מעולם לא הוגדרו → `manager-mail-oauth` מחזיר `not_configured` בכוונה (FAIL VISIBLE, הטוסט האדום שMike רואה). נמצא גם: `MANAGER_MAIL_ENABLED`/`MANAGER_DIGEST_ENABLED` חסרים היו משתיקים sync/digest cron בשקט גם אחרי חיבור Outlook עתידי — שניהם הודלקו עכשיו (`=true`, בטוח כי אין עדיין mailbox מחובר). Mike אישר שהמייל בפועל על Microsoft 365. נמסר checklist Azure App Registration מדויק (Web redirect `https://bunohsdggxyyzruubvcd.supabase.co/functions/v1/manager-mail-oauth/callback`, scopes Mail.Read/Mail.ReadWrite/Mail.Send/User.Read/offline_access). **ממתין:** Mike/IT ליצור את ה-App ב-Azure ולמסור client ID+secret+tenant ID; גם לוודא פרופיל אורית `orit_cs_agent_access=true`+`phone` מלא (נדרש ל-digest WhatsApp).) (session 140b — **deploy:** Orit CS Agent access — migration 156 `profiles.orit_cs_agent_access`+מתג ב-`UserManagement`; super_admin תמיד רואה טאב; אחרים רק אם מסומן.) (session 140 — **מקומי→deploy:** Orit CS Agent — migration 155 `orit_agent_*` (mailbox/threads/messages/auto-ack 72ש'/drafts/digest)+RLS+seed demo+cron; `_shared/oritAgentMail.ts`+`oritAgentAi.ts`+`microsoftGraph.ts`; functions: `manager-mail-oauth`/`manager-mail-sync`/`manager-mail-analyze`/`manager-mail-send`/`manager-morning-digest` (Whapi DM אישי לאורית); `OritCustomerServicePanel.js`+`auth.js` `canAccessOritCsAgent`+route `orit_cs_agent` (אורית `orit@dream-island.co.il`+super_admin); Outlook OAuth plug-and-play; build נקי. **לא deployed.**) (session 137b — **deployed:** כל insert ל-`guest_alerts` (לוח בקשות) שולח גם לקבוצת Whapi «בקשות אורחים» — `_shared/guestAlertWhapiNotify.ts` `onGuestAlertInserted`; חובר ב-webhook (כל נתיבי guest_alerts)+`inbox-route-request`+`guest-portal-upsell`+`guest-portal-spa-request`; אותו `resolveRequestsWhapiGroupId`.) (session 139 — **מקומי:** Group courtesy automation — migration 154 `guests.automation_scope` (full/courtesy_only/muted)+`merge_automation_scope`; remark occupant (שם+טלפון בהערות, coordNameDuplicated) → courtesy_only = Stage 4 (`mid_stay`) בלבד; רכז עירייה בלי דייר → muted; `room_ready`/manual exempt; `ArrivalImportPanel` toggle «אוטומציית נימוסים לקבוצות»+עמודת אוטומציה; `_shared/automationSchedule.ts`; functions: send/cron/webhook/queue; 179 tests, build נקי.) (session 138 — **מקומי:** Inbox תמונות אורח — migration 153 `message_type`/`media_url`/`media_caption`+bucket `wa_inbox_media`; `_shared/metaMedia.ts` Meta download→Storage; `whatsapp-webhook` ענף `image` (`intent=media_received`, בלי LLM); `WhatsAppInbox` בועת `<img>`+lightbox; 177 tests, build נקי.) (session 137 — **deployed 0515cba:** Inbox «🛎️ ללוח בקשות» — `inbox-route-request` Edge Function: `guest_alerts`+Red Alert+Whapi לקבוצת «בקשות אורחים» (`routing_config` `alert_inbox_routed` / `WHAPI_REQUESTS_GROUP_ID`)+DM אישי (`SLA_GUEST_ALERT_PHONE`); migration 152; receptionist רואה «לוח בקשות»; 177 tests, build נקי.) (session 136 — **deployed:** תיקון "כן, מגיעים!" חוזר — `canGuestConfirmArrival()` pre-check הוסר משני ה-branches (pre/post-burst); חזרה על אישור הגעה אחרי ששלב 2 כבר נשלח לא נופלת יותר ל-LLM חופשי (הזיה מחוץ-לסקריפט), `handleStage2ArrivalConfirmation()` תמיד מריץ ומטפל בחזרה בשקט כמו נתיב הכפתור. גם `EdgeRuntime.waitUntil()` הושלם ב-webhook (היה נחתך ע"י race — ראה למטה) — שומר את ה-isolate חי עד ש-`processAsync()` (סבב LLM+burst coalescing, 2-5s+) מסיים, במקום שה-isolate יקפא מיד אחרי המענה ל-Meta ויבלע את התשובה בשקט. `whatsapp-send`: `automation_muted` (אורח-קבוצה מושתק) כעת לא חוסם `room_ready`/manual triggers — אלה שיגורים חד-פעמיים מאושרי-צוות, לא רצף האוטומציה שהדגל נועד להשתיק. `housekeepingWaParse` (backend+frontend): ✅ מנצח תמיד ניסוח צ'ק-אין באותה שורה. ⚠️ **תקרית התנגשות-כלים אמיתית תועדה ותוקנה תוך כדי הסשן:** `.git/index.lock` תקוע (37 דק', תהליך מת) + אינדקס staged חלקי-כתיבה (webhook נחתך באמצע comment בלי סגירת `Deno.serve()`, שורת changelog חתוכה) — עדות ל-Cursor וה-session הזה נוגעים באותם קבצים בו-זמנית (ראה האזהרה הקיימת "ריבוי כלים על אותו repo" למטה). ננקה ה-lock אחרי אימות שאין תהליך חי מחזיק בו; ה-truncation נתפס ב-`git show --stat` מיד אחרי commit (ראה ספירת קבצים לא-צפויה) ותוקן ב-commit נפרד לפני push — קוד שבור לא הגיע ל-origin/main. build נקי, 22/22 suites 177/177 tests.) (session 135 — **מקומי:** Inbox read cursors + recent-at-top + per-room room_ready — migration 151; `inboxReadState.js`+`WhatsAppInbox` persistent unread + «🕐 פעילות אחרונה»; `whatsapp-send`/`suiteRoomReady.ts` roomId; AICopilot/SuitesDashboard/GuestsPage; 176 tests, build נקי.) (session 134d — **deployed 7368264:** Doc 2 שיבוץ סוויטות — enrich/שיבוץ-בלבד ממלאים `guests.room` רק אם ריק; עמודת «פעולת סנכרון»; מרובה-חדרים → `buildCombinedRoomLabel`+suite_rooms לכל שורה; 170 tests.) (session 134c — **deployed 9d8d404:** HK WA ↔ שער אישור מנהל (כמו טאבלט) — `housekeepingGuestLookup`+`roomPendingApprovalPush`; `14✅`→`ממתין לאישור`+push/ack עם שם אורח חי; AICopilot `arrival_date=היום`+`guestRoomMatchesSuiteId`; functions: whapi-webhook+room-pending-approval-notify.) (session 134b — **deployed 9d8d404:** AICopilot realtime INSERT+UPDATE.) (session 134 — **deployed f7a1fec:** Housekeeping WA ready+צ'ק אין.) (session 133b — **deployed 47e82a7:** `CANONICAL_STAY_CHANGE_HANDOFF_MSG` — הוסרו שמות צוות «אדיר ואפק» מהודעת handoff לאורח; נשאר «צוות הסוויטות שלנו» בלבד. functions: webhook+cron+automation-queue+notify-manual-task.) (session 133 — **deployed 2436c0e:** Smart Guest Profile ETA sync — `_shared/guestEta.ts` (`extractArrivalTimeFromText` עברית+אנגלית AM/PM, `persistGuestEta` gate `arrival_date>=היום` ישראל, Record-Only בלי alerts); webhook Tier-0 refactored ל-`persistGuestEta`; `pickGuestFromRows`→`israelYmd` (לא UTC); `GuestsPage` מציג 🕐 ETA בעמודת הגעה + `sortCheckinRosterGuests({prioritizeEta})` בסקופ «היום»; 158 tests, build נקי.) (session 132 — **deployed 2436c0e:** Human-in-the-Loop אישור לבקשות תפעול שטח (מגבות/מים/תחזוקה) — בעקבות ה-false-positive שתועד באותו יום (`26abc3b`), הבוט איבד את הסמכות לשגר אוטונומית ל-קבוצת הוואטסאפ הזרה; עכשיו רק יוצר `pending_approval`, וצוות מאשר/עורך/דוחה ב-ACC לפני שיגור בפועל. **migration 149:** `tasks.status` מתרחב ל-`pending_approval`/`rejected` + `reviewed_by`/`reviewed_at`/`dispatched_at`/`rejection_reason`; `sla_deadline` מחושב עכשיו מ-`dispatched_at` (זמן אישור), לא מזמן ההודעה המקורית (SLA משקף זמן-לביצוע מרגע שהמשימה ניתנת לפעולה). **`whatsapp-webhook`:** `routeGuestRequestToOpsGroup` פוצל — `createPendingOpsApprovalTask` (חדש, מחליף) יוצר רק שורת `pending_approval`, בלי תרגום/שיגור Whapi; שלושת נקודות הקריאה (Tier-0 pre-burst+post-burst ב-`handleOperationalInHouseIntercept`, ותור-הכלים LLM) עודכנו יחד. `buildOperationalDispatchReply` (`automationSchedule.ts`) — הוסר "בדרך אליכם לחדר" (הפך שקרי ברגע שהשיגור מותנה באישור אנושי) → "קיבלנו את הבקשה שלך, הצוות שלנו בודק ומטפל בה כעת 🙏". **`notify-manual-task` הורחב (לא function חדש, שימוש-חוזר מכוון)** לשמש גם כ"אשר ושגר": guarded conditional `UPDATE ... WHERE status='pending_approval'` כמוטציית ה-DB **הראשונה**, לפני תרגום/Whapi — כך ש-double-tap על "אשר" לא משגר כפול (Postgres MVCC מכריע מי מנצח); `[GUEST WA]` prefix נוסף ל-`source='guest_request'` (היה נופל ל-`[MANUAL TASK]` המטעה). **`OperationsBoard.js`:** טאב חדש «⏳ ממתינות לאישור» (pulse ויזואלי + default-select אוטומטי כש-count>0 בטעינה ראשונה בלבד — לא "גונב" למשתמש טאב אחר על realtime tick), תיאור הבקשה ניתן לעריכה בכרטיס `pending_approval` לפני אישור, כפתורי «✅ אשר ושגר»/«🗑️ דחה» (guarded reject update, סיבה אופציונלית, לעולם לא DELETE — Zero Data Loss). ⚠️ **באג אמיתי נמצא ותוקן תוך כדי audit (לא היה קשור ישירות לפיצ'ר, אך היה מתפוצץ ברגע הראשון שנוצרה שורת `pending_approval`):** `src/App.js`'s `openTasks`/`urgentTasks` (Dashboard tile "🛠️ תפעול") ו-`openOpsCount` (בדג' sidebar) השתמשו ב-`status !== "done"` (רשימת-החרגה) — היו סופרים גם `pending_approval` וגם `rejected` בתור "עבודה פתוחה". הוחלף לרשימת-הכללה מפורשת (`status==='open' || status==='in_progress'`), עקרון FAIL VISIBLE §0.3. **Scope מכוון, לא נגעתי:** `admin_reception_tasks` (ספא/קבלה), Requests Board (`guest_alerts` — בלונים/upsell/פורטל), ושאר כל האוטומציה — אף אחד מהם לא הגיע לקבוצה הזרה מלכתחילה, אז אין מה לגדר שם. תהליך: ארכיטקטורה תוכננה ב-Plan Mode (סוכן Plan מקביל אימת/תיקן את ה-migration convention ומצא את באג ה-App.js), אושרה ע"י Mike, יושמה לאחר מכן. `npm run build` נקי, `npm test` 19/19 suites 154/154 tests. ראה §10 session 132.) (session 131b — **מקומי:** ACC תזמון — מצב «לפי שלב» עם תאריך+שעה (שלב 1 / ייבוא מאוחר); `defaultStaffScheduleDateYmd`; 140 tests.) (session 131 — **deployed 6857522:** ACC תור חי — תזמון מאסיבי migration 148+`QueueBulkScheduleModal`+cron/queue; 138 tests.) (session 130 — **מקומי:** ייבוא קבוצות מהערות — migration 147: Tier-2 `sync_suite_arrivals` לא דורס אורח שני באותה הזמנה; `automation_muted` one-way בייבוא; 132 tests.) (session 129 — **deployed 6034c7c:** Doc 2 «השלמת פרופיל» — migration 146 `enrichOnly`; עמודת «הבדל מול DB»; 131 tests.) Audit קוד מלא (`WhatsAppInbox.js`, ~5300 שורות) לפני קוד: `fetchThreadHistory`/`mergeThreadRows`/`fetchSince` (session 125c) כבר עושים merge מוסיף-בלבד שלעולם לא זורק היסטוריה בפגישה חיה אחת — אין "מחיקה" אמיתית של הודעות. שני ממצאים אמיתיים: (א) `rosterSearch` הקיים סינן רק את מה שכבר בזיכרון (`fetchAll`'s חלון 400 השורות האחרונות **בכל האתר יחד**, לא per-guest) — אורח שלא היה פעיל לאחרונה, או מילת מפתח שהוזכרה רק בהודעה ישנה, לא היו ניתנים לאיתור כלל עד שנפתחים ידנית. (ב) `sendManualReply` לא כפה גלילה לתחתית — אם הצוות היה גלול למעלה לקרוא היסטוריה ואז שלח תשובה, ה-`nearBottomRef` guard (שנועד למנוע קפיצה על עדכון *נכנס*) חסם גם את הגלילה להודעה *היוצאת* של הצוות עצמו, כך שההודעה שנשלחה נחתה מתחת לגלילה בלי שום משוב חזותי — בניגוד לוואטסאפ אמיתי ששולח את המשתמש להודעה שלו-עצמו תמיד. **תוקן:** (1) `useEffect` חדש (debounced 400ms, min 2 תווים) שואל ישירות את ה-DB — `whatsapp_conversations.message ILIKE` (תוכן הודעה, לא רק מטא-דאטה) + `guests` name/phone/room ILIKE — ומאחד תוצאות דרך `mergeThreadRows`/`fetchThreadHistory` הקיימים (אותו cap `THREAD_HISTORY_LIMIT`, אותה additive-merge contract) כך שאורח שנמצא רק ב-DB מקבל את ההיסטוריה המלאה שלו בדיוק כמו פתיחה ידנית. placeholder + label חדש `dbSearching` מציגים "🔎 מחפש גם בהיסטוריה המלאה…" בזמן החיפוש; טקסט השדה עודכן לציין שהוא כולל תוכן הודעה. (2) `sendManualReply` קובע `nearBottomRef.current = true` באופן יזום (לפני ה-round-trip) כך שההודעה של הצוות עצמו תמיד גוללת לתחתית — תואם התנהגות וואטסאפ. `npm run build` נקי (575.54kB +415B), `npm test` 16/16 suites, 123/123 tests. **⚠️ לא נבדק חי בדפדפן** — login דמו (`eliad`/`1234`) נדחה ב-preview (אותה מגבלה מתועדת ב-sessions 47/51/52); Mike צריך לאמת עם login אמיתי. ראה §10 session 127.) (session 126 — **מקומי, לא deployed:** תיקון סיווג יום-כיף/פרימיום — `sync_suite_arrivals` (migration 144, מחליפה 135) מעולם לא הפיקה `premium_day_guest` וגם דרסה סיווג פרימיום קיים בחזרה ל-`day_guest` בכל re-sync (root cause ל"אורחי-יום רק פרימיום מיובאים בפועל"); עכשיו שומרת סיווג פרימיום קיים + מקדמת אוטומטית לפי שם חדר `Premium Day 1/2` (מראה `AddGuestModal.inferRoomType`) לנתיב הייבוא שכן נושא שם חדר. `guest-portal-order` (הזמנות מהפורטל) היה הפונקציה היחידה שנשארה קוראת `upsell_items.target_audience` הישן/קפוא — migration 097 עדכנה את כולם חוץ ממנה; פרימיום-יום-כיף היה יכול להזמין רק פריטי `target_audience='all'` — הוחלף ל-`visibility_settings` כמו `guest-portal-data`. `supabase/tests/sync_suite_arrivals_premium_test.sql` חדש (5 תרחישים, BEGIN/ROLLBACK). מנוע הכשירות/דיספאץ' עצמו (`automationSchedule.ts`/`suiteNames.ts`/`whatsapp-send`/`whatsapp-cron`) אומת תקין — `day_guest`/`premium_day_guest` כבר מטופלים זהה בכוונה שם. ⚠️ דגל ל-Mike, לא תוקן: כמה תבניות Meta ל-fallback יום-כיף חולקות שם עם תבניות הסוויטה (`morning_welcome`→`suite_welcome_morning`, `mid_stay_daypass`/`checkout_fb_daypass`→אותו שם כמו המקבילות בסוויטה) — רק כשחלון 24ש' סגור, לא נבדק תוכן; גם לא אומת מצב `is_active` חי של שלבי יום-הכיף ב-ACC. ראה §10 session 126.) (session 125c — **deployed:** P1+P2 Inbox+CRM: מיון roster «פעילות» טהור (הוסרה הצמדת לא-נקרא 19262d6 + עדיפות humanRequested — החלטת Mike); `mergeThreadRows` (exported, tested) — `fetchThreadHistory` עבר ל-DESC window + merge מוסיף-בלבד שלעולם לא זורק הודעות ושומר `_read` (הבאג: ascending+limit(1500)+replace ב-force זרק את החדשות בשיחות ארוכות); גלילת thread רק כשקרוב לתחתית (הוסר `lastUpdated` dep שמשך לתחתית על כל עדכון roster); שם אורח ב-`GuestsPage`/`GuestDashboard` פותח `openDreamBotChat` (כפתור 👤 חדש לפרופיל); טוסטים `duplicate_blocked` ב-`AICopilot` (waSkipNote בטוסט הסופי)+`SuitesDashboard` (ACC כבר מכוסה); `preloadPortalLinkMessage`/`preloadStage2ArrivalMessage` מחליפים שליחה ישירה — טיוטה ל-textarea (תבנית preloadRoomReadyMessage); סנכרון `checkinFilterStorage`↔Inbox chips (today↔in_resort, tomorrow↔מחר; seed רק כשיש ערך שמור). `WhatsAppInbox.test.js` חדש; 123 tests. **אומת חי:** בוט קולט ועונה תוך 5ש' אחרי ה-deploy; Inbox מציג.) (session 125 — **deployed:** P0 ניתוב סוויטות/יום-כיף + Inbox reactions. (A) `_shared/suiteNames.ts` חדש (מראה Deno של `suiteRegistry.js`) — `isEffectiveSuiteGuest`/`isEffectiveDayPassGuest`/`hasSuiteRoomTypeConflict`: חדר-סוויטה קנוני מנצח `room_type=day_guest` שגוי (שורש: `sync_suite_arrivals` כותב `room_type` מדגלי `isDayGuest`/`hasSuite` בנפרד מ-`room`). חווט ב-`checkEligibility` (`automationSchedule.ts` — applies_to לפי סיווג אפקטיבי), `whatsapp-cron` (SELECT+`room`), `whatsapp-send` (אזהרת `room_type_conflict` בלוג, שער חדש `suite_daypass_stage_gate` — סוויטה לעולם לא מקבלת night_before_daypass/morning_daypass/mid_stay_daypass/checkout_fb_daypass, day-pass gate/pre_arrival_2d override/morning_welcome fast-path לפי סיווג אפקטיבי), `automation-queue` (פולט `effectiveSuite`+`roomTypeConflict`), ACC (`isDayPassQueueItem` — צ'יפים 🏨/☀️+שערי bulk+ManualDispatchModal זהים לשרת; באדג' «⚠ סתירת סיווג»), `guestTiming.hasSuiteRoomTypeConflict` + באדג' ב-`GuestsPage` (עמודת סוג) ו-`GuestDashboard` (כרטיס). (B) `whatsapp-webhook` — branch ייעודי ל-`msg.type="reaction"` לפני ה-fallthrough הגנרי: «❤️ תגובה על ההודעה: «snippet»» (lookup ב-`whatsapp_conversations` לפי `reaction.message_id`, קטיעה 60 תווים; אימוג'י חסר=«הוסרה תגובה»), log-only+`continue` — אפס LLM. 117 tests, build נקי; functions: webhook/send/cron/queue/history.) (session 124e — **מקומי:** Inbox mount תמיד `fetchAll`+cache paint בלבד; `fetchSince` gte; emergency `fetchAll` בלי Realtime; migration 143 intent widen; webhook outbound insert error logs; 111 tests.) (session 124c — **מקומי:** תג עזיבה — `isGuestDeparted` מנצח `checked_in`.) (session 124b — **deployed 3dc192f:** Pulse «דורש טיפול» = human_requested.) (session 122 — **deployed f00bae6:** Guest delete full sync — RPC `delete_guest_profile` migrations 141+142; `_shared/guestOutboundGuard.ts`+חיבור send/webhook/cron; Inbox `classifyInboxContactSegment`+תג «לא רשום»+באנר+disable ✏️; pipeline log FAIL VISIBLE; 96 tests.) (session 121b — **מקומי:** Inbox סנכרון מחיקת אורח — realtime DELETE/INSERT על `guests`; `syncInboxContactWithGuestMap` מנקה «מחר»/פרופיל ישן; groupByPhone last-message-wins; 9 tests.) (session 121 — **מקומי:** Inbox פיקוח שיחה — «🔄 רענן היסטוריה»+מונה `X/Y הודעות ב-DB`+אזהרה כשחסר; רשימת אורחים — סינון (בריזורט/מחר/יומיים/עתיד/בטיפול/לא נקרא/התראות/עזיבה)+מיון (פעילות/הגעה/שם)+קיבוץ לפי סקשן ב«הכל»; `guestTiming.classifyInboxRosterSegment`; 93 tests.) (session 120 — **deployed:** אישור הגעה שלא נקלט — `_shared/arrivalConfirmation.ts`; webhook dedup-skip catch-up; cron inbox backfill לפני Stage 2 reconcile; functions: webhook/cron; 4 tests.) (session 119c — `CheckinTimelineFilterBar`+`useCheckinTimelineFilter`+`checkinFilterStorage` sessionStorage; `GuestsPage`+`GuestDashboard` מסונכרנים; `applyCheckinRosterFilter` ב-guestCheckinMatrix.) (session 119b — **deployed 4bc7e6c:** Inbox «🛎️ חדר מוכן» — preload textarea מ-room_ready_reminder/dream_room_ready לפני inbox_reply; Task 3.) (session 119 — **deployed e24ff25:** Queue→Inbox deep link+delivery badges; Stage 2 hotfix — early «כן מגיעים» intercept לפני auto-checkin, multi-row phone lookup, whatsapp-send pipeline fallback; functions: webhook/automation-queue.) (session 117c — **deployed 7475896:** Stage 2 reconcile באג קריטי — `event_immediate` לא נסרק; migration 138 מאפס דגל שגוי; cron בודק `notification_log` לא רק `msg_stage_2_arrival_sent`.) (session 117b — **deployed 269a497:** בוקר סוויטות — תזמון Asia/Jerusalem+migration 137 (06:00 לא 15:00); Stage 2 reconcile ב-cron לאורחים עם ✓ אישר בלי שלב 2; wa_window לפני שליחה; functions: webhook/cron/send/automation-queue.) (session 117 — **מקומי:** Stage 2 staff-claim fix — «כן מגיעים» עוקף claimed_by mute; flag רק אחרי send אמיתי; post-burst confirm לפני staff exit.) (session 116 — **deployed c75099c:** פרופיל אורח+פורטל — פנסיון (חצי/מלא/ערב)+שעות ארוחה; migration 136; סיכום שהייה פרטי/קבוצה+חדרים מ-suite_rooms; `meals_itinerary` בפורטל; 82 tests.) (session 115d — **hotfix migration 135:** `sync_suite_arrivals` `v_guest_id` UUID→BIGINT; תיקון שגיאת סנכרון `invalid input syntax for type uuid: "3334"`.) (session 115c — **deployed 1edc4b1:** multi-room display+sync dedupe — `חדר N מ־M`; profile אחד per order+phone+date; 76 tests.) (session 115 — **deployed 544fdef:** `spa_date` סנכרון מלא — `_shared/spaSchedule.ts`+migration 131; פורטל/Inbox/WA placeholders; 70 tests. **+ merged:** ייבוא opt-in migration 132; Stage 2 מיידי migration 133; functions redeployed.) (session 114 — AddGuestModal `spa_date`+`IsraeliTimeSelect` 24ש'.) (session 113 — **deployed f3bc4ca:** Inbox WYSIWYG שלב 1 — `_shared/metaTemplateLog.ts`+migration 130: גופי תבניות מלאים ל-night_before_suites/_shabbat ושאר pipeline; fallback Meta Graph API; כפתורי QR בלוג. functions: whatsapp-send/webhook/cron/automation-queue.) (session 112 — **pushed main 7cdf24e:** חיזוק מגן תשובה קטועה בשעות כניסה — `looksLikeCheckInHoursReply`+`resolveTruncatedReplyFallback`; migration 129 mid_stay; אוטומציה דטרמיניסטית cron→Meta template; Stage 2 webhook רק bot_scripts. functions: whatsapp-send/whatsapp-webhook/cron/automation-queue.) (session 111 — **deployed 43c87aa:** (1) Inbox unread — `countUnreadInbound()` סופר רק inbound בלי outbound אחריו; `markPhoneInboundRead` אחרי מענה צוות; רשימה ראשית מסתירה אורחים שעזבו (`isGuestDeparted`+chip «אחרי עזיבה»). (2) בוט: Tier-0 `isCheckInPolicyQuestion`+`buildCheckInPolicyReply` מ-`bot_config` night_before_*; `isReplyObviouslyTruncated`; Gemini מחבר כל חלקי טקסט. functions: webhook/cron/automation-queue.) (session 110 — **deployed 60619d0:** מובייל צ'ק-אין כרטיסים+overflow; BotConfigPanel פאנל שעות כניסה; whatsapp-send resolveDayTimings מ-bot_config.) (session 109c — **deployed 1c18236:** Inbox — הוסר צבע סגול לשורות בריזורט; תגית 🟢 בריזורט נשארת.) (session 109b — **deployed b1a3038:** תור חי שלב 2 גלוי; guest_request ops — חדר ב-Whapi, SLA, משק/תפעול; functions: webhook/cron/automation-queue/history.) (session 105 — **deployed:** Stage 2 בתור חי — migration 127, whatsapp-webhook/send/cron/automation-queue, ACC שגר מאסיבי.) (session 104b — **deployed 48e2314:** Inbox כפתור 🔁 «שלח שוב הודעת הגעה (שלב 2)» — `manual_script`+`stage_2_arrival` ב-`whatsapp-send` עם `resolveStage2ArrivalPlaceholders` מלא; `whatsapp-webhook` Stage 2 fix באותו commit.) (session 104 — `whatsapp-webhook`: «כן מגיעים» (כפתור/טקסט) תמיד שולח `stage_2_arrival` מ-BotScriptEditor; הוסר gate `!arrival_confirmed` שגרם לנפילה ל-LLM+הקשר ספא-פורטל («קיבלנו את הבקשה שלך לטיפול בספא»); `handleStage2ArrivalConfirmation`+`isArrivalConfirmationMessage` משותפים; post-burst intercept.) (session 103 — **deployed 2a918ad:** Inbox UX — `formatTime` מציג תמיד שעה (היום: HH:MM; ימים קודמים: DD/MM HH:MM + title מלא); כפתור 📱 «טלפון»+מודאל QR (`staffDeepLink.js`) לפתיחה מהירה בטלפון; deep link `?page=wa_inbox` (+אופציונלי `phone`) נתפס ב-`index.js`, נשמר ב-sessionStorage, מופעל אחרי login ב-`App.js`. build נקי.) (session 102b — **deployed a432dd3:** Stage 3 בוקר הגעה — אותו תיקון שבת כמו Stage 2.5: cron לא מנתב ל-stage_3_morning session בגלל חלון 24ש'; תמיד `suite_welcome_morning`/`_shabbat`; day-pass גם; fallback שבת→session+override במקום תבנית חול עם 15:00.) (session 102 — **deployed 812e942:** Stage 2.5 שבת — `whatsapp-send` cron לא מנתב יותר ל-session text רק כי חלון 24ש' פתוח (גרם ל-12:00/15:00 במקום תבנית `night_before_suites_shabbat`); `isShabbatArrivalDate`+`normalizeArrivalDateYmd` מרכזיים; `applySaturdayCheckInTimeOverride` מחזק session ידני; migration 126 ממלא bot_config שעות שבת 15:00/18:00.) (session 101 — **מקומי, לא deployed:** ייבוא Doc 2 — `duplicateCoordNameKeys`+`coordNameDuplicated` ב-`aggregateGuestProfiles`/`extractGuestDetails`: זהות מ-sRemark (שם+טלפון) **רק** כש-`sClientFullName` מופיע על 2+ שורות באותו קובץ (עיריית/קבוצה); שורה יחידה משתמשת בעמודות בלבד — תיקון באג שמשפט תפעולי בהערות («חוגגים יום הולדת לפנק») הפך לשם אורח. 56 tests, build נקי.) (session 100c — **deployed:** ייבוא Doc 2 — `extractNameFromRemarkWithoutPhone`+שיוך טלפון מ-sTel1; `ArrivalImportPanel` באנר/בדג' 📵+רשימת שמות ללא טלפון לפני/אחרי סנכרון; `preprocessCsvText` מתקן `ש"ח` ב-sRemark (קובץ 5.7.26 נשבר מ-24 ל-7 שורות). 55 tests, build נקי.) (session 100b — **deployed:** חילוץ חכם מ-sRemark — `trimRemarkNameNoise`+`pickOccupantNameFromPrefix` (אורח ליד טלפון ב-/+, חיתוך בחדר/שח/₪/פסיקים); `_isSuspiciousGuestName` מורחב; שורת רינת עקיבא→שם נקי. 53 tests, build נקי.) (session 100 — **deployed 795ad2e:** קבוצות מוניציפליות/ארגוניות עם טלפון מזמין דמה (`111` וכו') — `isDummyPhone()` הועבר ל-`ezgoParser.js` (מקור יחיד); `extractGuestDetails` לא משתמש בטלפון דמה כפרופיל אורח (הערות=sRemark מנצחות); `mergeCandidates` לא מנפח `roomsCount` לפי מספר שורות באותה הזמנה (גרם ל-⛔ umbrella שגוי); `classifyDbMatch` לא חוסם שורה עם זהות אמיתית מההערות גם כשהמזמין דמה — `automationMuted` נשאר (ללא WA אוטומטי). 50 tests, build נקי.) (session 99b — **מקומי, לא deployed:** Doc 2 sync accuracy — `resolveSuiteFromEzgoFields`/`roomsCanonicallyMatch` ב-`suiteRegistry.js` (מספר חדר + סוג סוויטה → «אמטיסט 8», Premium Day 1/2); `classifyDbMatch` לא מסמן ⚠ כשהקובץ שולח «8» וה-DB «אמטיסט 8»; גריד ממלא חדר אוטומטית; `handleSync` מעדכן `guests.room` scoped ל-`arrival_date` אחרי RPC; migration 125 מתקן את אותו scope ב-`sync_suite_arrivals`. 48 tests, build נקי.) (session 99 — `MappingReviewPanel` עמודת «ברירת מחדל (ריקות בלבד)» editable לשעות (`checkinTime`/`checkoutTime`) + שדות `defaultPolicy`; ולידציה HH:MM; `applyFieldDefaultsToProfiles`/`parseMappingMemory`/`packMappingMemory` v2 ב-`importMapper.js`; `handleMappingApprove` ממלא ריקות אחרי `aggregateGuestProfiles` ושומר `fieldDefaults` ב-`import_mapping_memory`; `detectEzgoArrivalsPreset` ממפה `sCheckInTime`/`sCheckOutTime` כשקיימים; `EditableGrid.cellEditable` per-row. מסלול A (CSV quote-aware + ⚠ שם שבור) כבר ב-`c558760`. 6 בדיקות `importMapper.test.js`; `npm test` 26/26, `npm run build` נקי. QA Mike: Doc 2 עיריית + שורה 6 + 15:00/11:00 במיפוי.) (session 98 — **מקומי, לא deployed (frontend-only, אין supabase/):** Guest Import Intelligence Sprint 3 — `mergeCandidates`/`classifyDbMatch` (`guestImportIntelligence.js`) חוברו בפועל ל-`handleSync()` ב-`ArrivalImportPanel.js`, לא רק לבדג' תצוגה: DB prefetch חדש (`existingGuestsMap`, שאילתה אחת לכל תאריכי-הגעה בסשן, keyed phone+date/order+date) מזין `dbMatchByIdx` (new/existing/conflict/unimportable אמיתי, לא unimportable-או-כלום כמו קודם) — הגריד מציג עכשיו 🆕/🔄/⚠/⛔ חי. `_getSyncProfileIndices` (הפך `export`) מדלג על unimportable, **לא** מדלג על conflict (מסונכרן + מדווח בנפרד), מחזיר `{indices,conflicts,skippedUnimportable}`. `handleSync`'s profiles/rooms builders + לולאת ה-patch אחרי ה-RPC קוראים עכשיו את `mergedCandidates[i]` (זהות/ספא/ארוחה/lead_source/automation_muted ממוזגים) כמקור-אמת במקום `merged[i]` הגולמי — עריכות גריד (`edited.*`) עדיין מנצחות את שניהם. תוצאת הסנכרון מציגה שורות אזהרה חדשות (⛔ דולגו / ⚠ התנגשות) באותה קונבנציית amber כמו `skippedRooms` הקיים. 9 בדיקות חדשות (`guestImportIntelligence.test.js`+`ArrivalImportPanel.test.js` חדש) — `npm test` 28/28, `npm run build` נקי. ⚠️ ידוע ולא-תוקן: `classifyDbMatch`'s roomConflict משווה `room` הגולמי מה-CSV מול הפורמט השמור ב-`guests.room` — עלול לייצר "conflict" כוזב על ייבוא חוזר של אותו קובץ (לא חוסם, רק מסמן). ראה §10 session 98.) (session 97 — **מקומי, לא deployed:** Sensitive Stay-Change Shield regex gap fix — `SENSITIVE_STAY_CHANGE_PATTERN` (`_shared/automationSchedule.ts`) דרש צמידות ישירה בין "חדר"/"מוקדם" ובין "לצאת"/"מאוחר" — ניסוח אמיתי מהשטח ("רציתי לדעת אם יש אפשרות לקבל את החדר יותר מוקדם") לא נתפס כי המילים מופרדות במשפט, וההודעה נפלה דרך כל שלושת ה-Tier-0 classifiers (לא complaint/upsell/stay-change) עד ל-branch ה-"faq" (LLM חופשי) — שם `buildGuestStageContext()` מזריק תמיד את שעת הספא של האורח להקשר, מה שסביר שהטה את התשובה לספא במקום התייחסות לבקשת החדר. תוקן: שני האלטרנטיבות הורחבו ל-gap-tolerant (`[\s\S]{0,20-25}`) במקום `\s*` הצמוד — "חדר...מוקדם"/"מוקדם...חדר"/"להיכנס מוקדם" ו-"לצאת...מאוחר" נתפסים כעת גם כשמופרדים במילים אחרות, בלי לפגוע בבדיקות FAQ תקינות (נבדק מול 13 מקרי קצה). ההודעה תיעצר כעת ב-Tier-0 (0 עלות LLM) ותקבל את ה-`CANONICAL_STAY_CHANGE_HANDOFF_MSG` הקבוע + `attention_reason="date_change"` (🗓️ ב-badge) כמו כל בקשת שינוי-שהות אחרת. `npm run build` נקי. ראה §10 session 97.) (session 96 — **מקומי, לא deployed:** Manual Portal Link Dispatch — migration 124 `bot_scripts` row `manual_portal_link` (`trigger_event='manual'`, לא נגוע ע"י שום automation_stages/cron scan); `whatsapp-send` BRANCH F (`trigger:"manual_script"`, `{guestId, scriptKey}`) — resolver ייעודי `resolveManualScriptPlaceholders()` (GUEST_NAME+portal_url/PORTAL_LINK, אותו graceful-fallback contract) + 24h window guard כמו BRANCH C, נוסף ל-`MANUAL_TRIGGERS`; `WhatsAppInbox.js` כפתור "🔗 שלח קישור לפורטל האורחים" בפאנל הפעולות המהירות — קליק אחד, ללא textarea, מנוטרל+title כש-`activeContact.guestId` חסר (Disable Don't Hide §0.2). `npm run build` נקי.) (session 95 — **מקומי, לא deployed:** Stage 2 Pay placeholder-leak fix (split-brain bug) — `resolvePaymentPlaceholders()` (`whatsapp-webhook`) טיפל רק ב-GUEST_NAME/PAYMENT_AMOUNT/PAYMENT_LINK/WORKSHOP_URL/SPA_LINE, בלי `{{SPA_TIME}}`/`{{OPTIONAL_SPA_TEXT}}`/`{{PORTAL_LINK}}`/`{{portal_url}}` (שכן כן מטופלים ב-`resolvePlaceholders()` הרגיל של stage_2_arrival) — נוסף טיפול זהה + `payPortalLink` הוזרק לקריאה. בנוסף: `sendCtaUrlButton`/`sendInteractiveButtons` (`_shared/interactiveSend.ts`) היו שתי פונקציות השליחה היחידות שדילגו על ה-Output Leakage Wall (`sanitizeReply()`'s final `{{...}}` strip, session 88 §Layer 4) — POST-ו bodyText גולמי ישירות ל-Meta ללא שום ניקוי. נוסף `_stripUnresolvedPlaceholders()` (מקומי לקובץ, כמו `_isAbortError`) בשתיהן. `sendStage2PayReply()` עכשיו מריץ `sanitizeReply(paymentReply)` פעם אחת ל-`finalPaymentReply` יחיד שמוזרק גם ל-Meta send (`sendCtaUrlButton`/`sendReply`) וגם ל-`whatsapp_conversations` log — אותה מחרוזת בדיוק לשני הצדדים, לא עוד עותק לא-מסונן ללוג ועותק אחר לשליחה בפועל. `_shared/interactiveSend.ts` משותף גם ל-`whatsapp-send`+`staff-ops-webhook` — דורש redeploy לשלושתם. ראה §10 session 95.) (session 94 — **מקומי, לא deployed:** Guest 360° Context Drawer — `GuestContextDrawer.js` + לחיצה על שם/אווטאר ב-`WhatsAppInbox` roster; migration 123 `staff_color_label`+`internal_notes`; השתקת בוט/אוטומציה, 5 משימות אחרונות; `npm run build` נקי.) (session 93 — **deployed d2dbaec:** Saturday session-script override — `applySaturdayCheckInTimeOverride()` ב-`whatsapp-send`: כש-`usedSessionMessage` ו-`arrival_date` בשבת (`getUTCDay()===6`), מחליף `15:00`→`18:00` ב-`bot_scripts.message_text` אחרי placeholders, בלי עריכת DB.) (session 92 — **מקומי, לא deployed:** Dispatch transparency — `whatsapp-send` לוג inbox מדויק `[META]`/`[SESSION]`+footer כפתורים; fallback body מ-`message_templates`/`TEMPLATE_BODY_APPROVED` (לא bot_scripts); Whapi admin alert על `failed`/`blocked_by_meta` (`ADMIN_PHONE_NUMBER`→`SLA_GUEST_ALERT_PHONE`→972546294885); `WhatsAppInbox.js` 🔵/🟢+`[+ Interactive Buttons]` בבועות יוצאות.) (session 91b — **מקומי, לא deployed:** בקשת בלונים לחדר — `isBalloonRoomRequest`/`buildBalloonRoomRequestReply` ב-`automationSchedule.ts`; Tier-0 intercept ב-`whatsapp-webhook` → `guest_alerts` בלבד (לא tasks/Whapi תפעול); תשובה קבועה «הקבלה תעביר פרטים לנציגת הבלונים»; secret אופציונלי `BALLOON_VENDOR_PHONE` ל-heads-up Whapi.) (session 91 — **deployed 8c6d33d:** Sidebar nav — «לוח בקשות» מתחת לצ'קליסטים מעל תפעול ואחזקה.) (session 90 — **deployed 8279bf6:** Operations Channel EN translation-layer gap closure — `translateTextForFieldOps`/`containsHebrew` (`_shared/fieldOpsTranslation.ts`, כבר קיים) חוברו לשלוש נקודות דיספאץ' לקבוצת "קריאות" שעד כה שלחו `tasks.description` גולמי: `whapi-webhook`'s `buildTaskCard` (Tier-0 regex), `sla-escalation-cron`'s ops-group SLA breach card, `task-action`'s Bump+Accept/Complete echo. `tasks.description` ב-DB לא נגוע — תרגום לכרטיס Whapi בלבד. `guest-portal-ops-request` (ערוץ "requests" נפרד) נבדק ונשאר ללא שינוי בכוונה. ראה §10 session 90.) (session 89 — **מקומי, לא deployed:** Staff-claim mute sync — `refreshStaffClaimMuteFromDb` אחרי burst 1.8s + early exit; `insertGuestOutboundIfNotMuted` (אין ghost ב-Inbox); `checkEligibility`+`whatsapp-send`+`whatsapp-cron` חוסמים cron כש-`claimed_by` פעיל; `WhatsAppInbox.setClaim` FAIL VISIBLE על 0 שורות; `npm run build` נקי.) (session 88 — **מקומי, לא deployed:** Webhook Defensive Shield audit — בוצע audit מלא של `whatsapp-webhook` מול blueprint 4 שכבות (Staff Override / Defensive Shield / Hard-coded Interceptions / Output Leakage Wall). Layers 1+3+4 נמצאו **כבר ממומשים** (claimed_by mute, severe-complaint kill-switch, sensitive stay/financial shields, operational/administrative intercepts, hard-drop guard) — תועדו כאן לראשונה כי היו committed בלי כניסה בהיסטוריית הסשנים. נוסף Layer 2 בפועל: `isLowValueCourtesyMessage()` (`automationSchedule.ts`) — הודעת אמוג'י-בלבד/מילת-נימוס בודדת (תודה/הבנתי/סגור/היי/אוקי) → `handleCourtesyAck` יוצא בשקט, בלי fallback script, גם pre-burst וגם post-burst. `LOW_CONFIDENCE_HANDOFF_SENTENCE` חולצה לקבוע יחיד ומוזרקת לשני מקורות הפרומפט; כש-`reply` מה-LLM שווה למשפט הזה (=המודל הודה שאינו יודע) — נכתב `requires_attention`+`attention_reason='שאלה מורכבת לצוות'` והשליחה לאורח **מדולגת** (staff לוקח את זה מהבדג' האדום, לא הודעת "אני בודק" חוזרת). `GuestAttentionBadge.js` קיבל אייקון 🤔 לסיבה החדשה. `npm run build` נקי. ראה §10 session 88.) (session 87 — **מקומי, לא deployed:** `suiteCheckinSync.js` — סנכרון צ'ק-אין `guests.checked_in`↔`room_status.תפוס`; `GuestsPage`+`RoomBoard`+`AICopilot`; אישור+שליחה→`room_ready` בלבד; «צ'ק-אין בלי הודעה»+«סגור שער» במקום «התעלם».) (session 86 — **deployed 8abed52:** AICopilot stale «ממתין לאישור» — סינון `room_ready_notified`/`msg_room_ready_sent`; ניקוי room_status→פנוי; `whatsapp-send` `clearPendingRoomApprovalGate` אחרי room_ready.)(session 84c — **deployed 5e043d1:** Live staff tracking — migration 120 `staff_members`+`room_status.current_*_name`; `AdminPanel.js` טאב «צוות ניקיון»; `HousekeepingTabletView.js` picker+תוויות חיות.)(session 84b — **deployed 5e043d1:** `room_ready_notified` migration 119 + idempotency guard.)(session 84 — **deployed 8535cfe:** Jacuzzi ping-pong workflow — `HousekeepingTabletView.js` סטטוסים `ממתין לג'קוזי`/`מוכן לפיניש`, כפתור «קרא לג'קוזי» בבניקיון, auto-handoff בג'קוזי נקי, «סיימתי פיניש»→ממתין לאישור+push; `RoomBoard.js` FAIL VISIBLE; migration 118.)(session 83 — **מקומי, לא deployed:** `ArrivalImportPanel.js` Doc 2 «עדכון שיבוץ סוויטות בלבד» — toggle cream+gold; `_executeSuiteAssignmentOnlySync` UPDATE ל-`room` בלבד לפי `order_number` או `name`+`arrival_date`.)(session 82c — **מקומי, לא deployed:** `WhatsAppInbox` «קח שיחה» → `guests.claimed_by` משתיק בוט לאורח ב-`whatsapp-webhook` (sendReply+Stage2Pay); תיקון groupByPhone שלא איפס claim בהודעה נכנסת; באנר 🔇 ב-thread.)(session 82b — `receptionChecklistTemplate.js` 21 משימות verbatim; `ReceptionChecklist.jsx` דשבורד ילנה+איפוס 04:00 — deployed b01c74d.)(session 80 — **מקומי, לא deployed:** פורטל אורח — `system_settings.enable_spa_request_button`+`has_spa_booking` מסתירים כפתור ספא; `guest-portal-spa-request` שולח תשובת Concierge Meta; migration 115.)(session 79 — סינון הכל/עם חדרים/ללא חדרים, כפתור ייבוא דינמי לפי סגמנט, `profile_type`+`profile_batch_type` ב-sync payload; `detailedReservationParser` מסווג לפי `rooms_count`.)(session 78 — **מקומי, לא deployed:** Reception Matrix — `GuestsPage`+`guestCheckinMatrix.js` (רשימה פעילה/ארכיון «אורחים לאחר שהות», auto 15:00+checkout, `checked_out` migration 114); `ReceptionChecklist.jsx`+`reception_checklist_entries` (3 אקורדיונים, audit+04:00 shift); `whatsapp-cron` auto_checkout; `notify-manual-task` תרגום EN רק תפעול/משק; Phase 2a Dashboard נשמר.)(session 77c — **Staff UI Phase 0+1 (מקומי, לא committed):** `App.js` tokens+utilities; `WhatsAppInbox.js` roster/CTAs/reply bar — Phase 0 vars, touch ≥44px, badge nowrap, swipe 48px, sticky reply+safe-area.)(session 77 — **Unified routing matrix + 15:00 auto check-in:** `automationSchedule.ts` — `shouldAutoPromoteToCheckedIn`/`resolveEffectiveGuestStatus`/`israelYmd`; מילות מפתח תפעול מורחבות; `ADMIN_REQUESTS_DEPARTMENT=קבלה/בקשות` + `shouldInterceptAdministrativeInHouseRequest`; תשובת שטח קבועה; `CANONICAL_STAY_CHANGE_HANDOFF_MSG` (אדיר ואפק). `whatsapp-cron` — bulk `pending`/`expected`→`checked_in` אחרי 15:00. `whatsapp-webhook` — gateway זהה; ניתוב תפעול→Whapi+Gemini EN; אדמין→tasks בלבד; stay-change→tasks+guest_alerts.)(session 76b — `isSensitiveStayChangeRequest` + `CANONICAL_STAY_CHANGE_HANDOFF_MSG`; late checkout/הארכה/צק-אין מוקדם → handoff ניטרלי (אדיר ואפק), `needs_callback`+`attention_reason=date_change`; הוסר מ-UPSELL_PATTERNS; pre_send_guard אם LLM דלף.)(session 76 — **Tier-0 operational intercept:** `automationSchedule.ts` — `OPERATIONAL_IN_HOUSE_KEYWORD_PATTERN` מורחב; `shouldInterceptOperationalInHouseRequest` + `buildOperationalDispatchReply`; `whatsapp-webhook` — אורח `checked_in` + מילת מפתח חדר → `routeGuestRequestToOpsGroup` + `requires_attention`/`attention_reason` + תשובה דטרמיניסטית לפני burst/LLM, אחרי dedup `wa_message_id`.)(session 75 — **Housekeeping→Manager alert:** migration 113 `room_status`→realtime; `room-pending-approval-notify` push להנהלה כשחדר+ג'קוזי נקיים ב-`HousekeepingTabletView`; AICopilot פעמון 🔔 לאישור הודעה+צ'ק-אין.)(session 74 — **Agent Playbook:** `docs/xos_agent_playbook.md` — תקשורת חסכונית, UI phases 0–3, desktop-first, living-doc protocol; `.cursorrules` מפנה אליו.)(session 73b — **xos_ai_rules edit/delete:** migration 112 RLS UPDATE/DELETE; `BotSettings.js` כפתורי ✏️/🗑️ לכללים שנלמדו.)(session 73 — **AI Learning + anti-reasoning seal:** `BotSettings.js` מציג `xos_ai_rules` (chat/routing); `whatsapp-webhook` — הפרדת suffixes, `ANTI_REASONING_LEAK_SUFFIX` אחרון ב-`enrichedPrompt`, מגן עברית ב-`sanitizeReply`; ללא שינוי `bot_settings.system_prompt`.)(session 72 — **Ops Board tasks Realtime:** `OperationsBoard.js` — `applyTaskRowUpdate` + channel `ops-board-tasks-rt` על `tasks` INSERT/UPDATE; 👍🏼 בקבוצת Whapi מעביר ל«בוצע» בלי רענון; migration 111 `tasks`→`supabase_realtime`.)(session 71 — **In-room context override + burst dedup:** `whatsapp-webhook` — מילות מפתח בחדר (מגבות/שמפו/מים/…) על `pending`/`expected` → `checked_in` async + `IN_HOUSE_TONE_SUFFIX`/`forceInHouse` ב-`buildGuestStageContext`; insert-first `wa_message_id` claim + burst coalesce ~1.8s למניעת כפל תשובות; helpers ב-`automationSchedule.ts`.)(session 69b — **Inbox in-resort purple roster:** `WhatsAppInbox.js` + `isGuestInResortToday()` ב-`guestTiming.js` — אורחים בשהייה פעילה היום מודגשים בסגול ברשימת DREAM BOT.)(session 69 — **Inbox route → Whapi group:** `WhatsAppInbox.js` `routeTask` קורא ל-`notify-manual-task` אחרי insert (`inbox_routed`); כרטיס `[GUEST WA]` בקבוצת `WHAPI_GROUP_ID`, 👍🏼 reaction כמו משימה ידנית.)(session 68 — **Queue per-stage select-all + Meta pulse:** `AutomationControlCenter.js` — צ'יפים «בחירה מהירה לפי שלב» בכל כרטיס יום הגעה; שגר המוני `BULK_SEND_PULSE_MS=2500` כמו `whatsapp-cron`.)(session 67 — **Stage 4 check-in gate toggle:** migration 110 `require_checked_in` על `automation_stages`; מתג במסע האורח ל-`mid_stay`; `automationSchedule` מכבד את הדגל; ברירת-מחדל OFF ל-mid_stay.)(session 66c — **תור חי לפי יום הגעה + Stage 4 fix:** קיבוץ אורחים לפי `arrivalDate`; `automation-queue` — `not_checked_in` נשאר `pending`; ימים עברו מקופלים.)(session 66b — **Suite spa-only Doc 1 sync:** `ArrivalImportPanel.js` — מצב ברירת-מחדל «ספא סוויטות בלבד» מפרסר רק שורות «לאורחי הסוויטות»; סנכרון ל-`guests` לפי `order_number`+`arrival_date` (פרופיל קיים בלבד); מצב «עדכון מלא» נשאר.)(session 66 — **Import spa filter:** `ArrivalImportPanel.js` — ברירת-מחדל «רק עם ספא» + toggle «הצג את כל האורחים»; `gridRows` מלא לסנכרון, `displayGridRows` מסונן לתצוגה; sync דרך `_profileIdx`. `ezgoParser.enrichProfilesFromExcel` — fallback join לפי טלפון כש-order_number לא תואם.)(session 65 — **Queue↔Timeline sync:** `AutomationControlCenter.js` — `mergeQueueWithStages`/`isCronScheduledStage` מסנכרנים תור חי עם `automation_stages` (is_active + שמות); `scheduleQueueRefresh` אחרי `patchStage`; פעימה מפרידה שלבי cron מ-`event_immediate`.)(session 64 — **Queue tab UX:** `AutomationControlCenter.js` תור חי מסנן `sent`/`simulated`/`skipped` (היסטוריה בלבד בטאב "מה נשלח"); 🗑️ ניקוי שורות `blocked_by_meta` — UI-only דרך `dismissedAttentionKeys`, ללא שינוי backend.)(session 63 — **Autonomous Deploy Protocol:** בסוף כל סשן קוד — תמיד להציע commit+push+db push+functions deploy ולהריץ באישור Mike; §12+§13+.cursorrules.)(session 62 — **Smart Guest Profile:** `guests.guest_profile` JSONB (migration 109); `GuestProfileModal` + `guestProfileSchema.js`; `GuestAttentionBadge` → מודל מובנה (VIP/אירוע/תזונה/הגעה/ETA); `guest_notes` = לוג מערכת בלבד; inbox banner הוסר; webhook `buildGuestStageContext` קורא פרופיל.)(session 61 — **WhatsApp Inbox UX:** באנר `guest_notes` מקופל כברירת-מחדל + כפתור ✕; `minHeight:0` על אזור ההודעות תיקן חפיפה; בועות מעט גדולות יותר. ראה §3 `WhatsAppInbox.js`.)(session 60 — **Receptionist RBAC refresh** + **Record-Only ETA** + **Future Suite Management routing:** `receptionist` = full Sidebar (`auth.js` `canSeeNavItem`/`canAccessRoute`); `guests.arrival_time` (migration 108) record-only webhook path; future suite asks → `120363429859248777@g.us` + LLM Hebrew→English before Whapi. ראה §4/§6/§7.)(session 59 — **needs_callback decouple:** דגל UI בלבד, בוט+cron לא נחסמים; AddGuestModal "ממתין לטיפול צוות 🔴"; §12 Deploy checklist חובה. ראה §10 session 59.)(session 57 — **System Audit & Cleanup.** (1) Security purge: הוסרו `MOCK_USERS` array + admin bypass block מ-`App.js`, display table ו-prop מ-`AdminPanel.js`, hardcoded default password `"1234"` מ-`invite-user/index.ts` (replaced by enforced 8-char minimum). (2) Bug fix — `guest-portal-data`: PostgREST `cs` DB-level filter הוחלף בסינון JS דו-שלבי: Level 1 (scene visibility) → Level 2 (CTA visibility per kept scene) — תוקן באג שהפיל סצנות שלמות ל-day_guest במקום לסנן רק CTAs פנימיים. (3) Day-pass Stage 5 audit: `checkout_fb` כבר היה ב-`DAY_PASS_ALLOWED_TRIGGERS` ו-`DAY_PASS_ALLOWED_STAGES` — תוקן רק טקסט "שלושה שלבים בלבד" → "ארבעה שלבים" + comment בקוד. (4) Dead code: הוסר משתנה `predictedChannel` לא-בשימוש ב-`AutomationControlCenter.js` — build `Compiled successfully.` ללא warnings. ראה §10 session 57.)(session 56 — **Master template variable sync** (`dream_suite_reminder` + `dream_welcome_morning`). שלושה תיקונים ב-`whatsapp-send/index.ts`: (1) `morning_suite`/`morning_welcome` לא העבירו משתנים `{{2}}`/`{{3}}` כלל — נוסף `resolveDayTimings()` synchronous: ראשון–שישי → 12:00/15:00, שבת → 15:00/18:00, מוזרק ל-`PIPELINE_VARS` של שני הטריגרים. (2) `portalButtonParam` היה night_before בלבד — הורחב ל-`PORTAL_BUTTON_TRIGGERS = {night_before, morning_suite, morning_welcome}` כך שגם הודעות בוקר-ההגעה מזריקות את `portal_token` לכפתור ה-URL. (3) `resolveNightBeforeTimes()` זרק שגיאה כשמפתחות שבת ב-`bot_config` ריקים — הוחלף ב-warn + fallback 15:00/18:00 כך שאורח שמגיע בשבת מקבל הודעה במקום כלום. `npm run build` נקי, נפרס לפרודקשן, pushed (c75a919). ראה §10 session 56.)(session 55 — **Meta template IMAGE header fix.** `sendViaTemplate` (whatsapp-send) ו-`sendTemplate` (whatsapp-webhook) לא כללו את רכיב ה-`header` ב-components payload — Meta זרק "Format mismatch, expected IMAGE, received UNKNOWN" לכל תבנית עם Media Header כגון `dream_suite_reminder`. תיקון: `TEMPLATE_IMAGE_HEADERS` map חדש בכל פונקציה — mapping שם-תבנית → URL תמונה; אם קיים, רכיב header מוזרק ראשון לפני body/button. נפרס לפרודקשן. ראה §10 session 55. )(session 54 — **Voucher Reconciliation Engine — synthetic validation + `voucher_numbers_match()` bugfix.** Migration 092 תוקן באג בפונקציה: `truncate_4` mode השתמש ב-`left(v_easygo_norm, length-4)` שחתך 4 *תווים* אחרונים — כשיש מפריד (e.g. `'999888-4321'`) נשאר `'999888-'` (עם מקף) שלא תאם את `'999888'` של הספק. תיקון: שלב נרמול חדש `regexp_replace(..., '[^A-Z0-9]', '', 'g')` על שני הצדדים לפני הקטיעה — מיישר `'999888-4321'`→`'9998884321'`→`'999888'` ✅. 8 בדיקות inline ב-migration עצמו (`DO $$ ... $$`) אוכפות את הנכונות בזמן deploy. נוסף גם `supabase/tests/voucher_reconciliation_e2e_test.sql`: 5 תרחישים מלאים (INSERT→RPC→assert→ROLLBACK) שניתן להריץ ב-Supabase SQL Editor בכל עת. `exact` mode (Hever/Nofshonit) לא שונה — בדיקה B מוכיחה שמפריד גורם לאי-התאמה כצפוי. ראה §10 session 54 לפירוט.) (session 53 — **Real-time Meta template sync buttons** נוספו ל-`WhatsAppInbox.js` (NewChatModal) ו-`BroadcastDashboard.js`: כפתור "🔄 סנכרן תבניות" שמאפשר לסגל לרענן את רשימת התבניות המאושרות מ-Meta בלי לרענן את הדף. `fetchTemplates` חולץ ל-`useCallback` בשני הקבצים. `npm run build` נקי. committed+pushed (8251e4a). ראה §10 session 53 לפירוט.) (session 52 — **RESORT_UI_MANIFEST.md** נוצר (root) כמקור-אמת נפרד ל-UI/UX philosophy + tab-readiness checklist, ואז **automated repair pass** סגר 7 מתוך הממצאים שהוא תיעד: silent/misleading error states ב-`AdminPanel.js`(×2)/`WhatsAppInbox.js`(×2)/`AutomationControlCenter.js`/`InventoryImportPanel.js` + tablet-grid layout gaps ב-`RoomBoard.js`/`HousekeepingTabletView.js`/`AICopilot.js`. `npm run build` נקי אחרי כל קובץ. CSS-variable drift (~150 hardcoded hex) ו-`AutomationControlCenter`'s tablet media-query נשארו **בכוונה** לא-מטופלים (out of scope, לפי הנחיית Mike). ראה §10 session 52 לפירוט מלא + §0 בהמשך לסשן 51 שנשאר מתועד למטה.)
>
...
הארכת שהייה / late checkout / צק-אין מוקדם / שינוי חדר:
  ...
  ✅ תשובה קנונית בלבד: CANONICAL_STAY_CHANGE_HANDOFF_MSG (ללא שמות צוות)
...
#### session 132 — 2026-07-07 (Human-in-the-Loop approval gate — operational field-ops dispatch)
> הקשר: אותו יום דווח false-positive (`26abc3b`, Hebrew word-boundary bug) בו הבוט שיגר אוטונומית בקשת שירות שגויה לקבוצת הצוות הזרה. Mike ביקש שינוי ארכיטקטוני: הבוט ימשיך לזהות בקשות תפעול שטח (מגבות/מים/תחזוקה), אבל יאבד את הסמכות לשגר לבד — צוות חייב לאשר (ואפשר לערוך) לפני שהתרגום+השיגור בפועל קורים. סוכם מראש (Plan Mode + 4 החלטות מפורשות עם Mike): scope מצומצם רק ל-`operational_field_ops` route (לא ספא/קבלה, לא Requests Board); טון תשובה חדש לאורח ("קיבלנו... בודק ומטפל" במקום "בדרך אליכם"); שעון ה-SLA להשלמה מתחיל מזמן-האישור (לא מזמן ההודעה); אין escalation אקטיבי על תור-האישור עצמו בסבב הזה (רק הבדג' האדום הקיים).

- ✅ **migration 149** — `tasks.status` CHECK מתרחב ל-`('pending_approval','open','in_progress','done','rejected')`; עמודות חדשות `reviewed_by`(UUID→profiles)/`reviewed_at`/`dispatched_at`/`rejection_reason`; אינדקס חלקי `idx_tasks_pending_approval`; inline self-test (`DO $$...RAISE EXCEPTION`) — נמצא ואומת ישירות ש-migrations 100/139 (על `notification_log`, לא `tasks`) כן משתמשות בדפוס הזה, אז זו לא המצאה — שילוב של שתי קונבנציות אמיתיות שקיימות בריפו (comment-style של 071/077 + self-test של 100/139).
- ✅ **`whatsapp-webhook/index.ts`** — `routeGuestRequestToOpsGroup` (עשה insert+תרגום+שיגור אטומית) פוצל: `createPendingOpsApprovalTask` (חדש, מחליף) עושה רק insert עם `status:"pending_approval"`, `sla_deadline:null` — בלי `resolveRouting`/תרגום/`sendWhapiText` בכלל. שלושת נקודות הקריאה (Tier-0 `handleOperationalInHouseIntercept` — pre-burst ו-post-burst, גוף פונקציה אחד; ותור-כלים ה-LLM `log_guest_request`) הוחלפו לקרוא לפונקציה החדשה, ללא שינוי ארגומנטים. `guests.requires_attention`/`attention_reason` **לא נגעתי** — נשאר כפי שהיה (הבדג' האדום הקיים ב-Inbox/GuestsPage ממשיך לפעול ללא שינוי, זו נקודת-הנראות הפסיבית שסוכם עליה). ייבוא `translateGuestRequestForFieldOps`/`resolveRouting`/`taskIntentType`/`buildGuestOpsSlaDeadline` הוסרו (הפכו dead code בקובץ הזה).
- ✅ **`_shared/automationSchedule.ts`** — `buildOperationalDispatchReply()`: "הבקשה הועברה ישירות לצוות השטח שלנו והם בדרך אליכם לחדר!..." (הפך שקרי) → "קיבלנו את הבקשה שלך, הצוות שלנו בודק ומטפל בה כעת 🙏".
- ✅ **`notify-manual-task/index.ts` הורחב (לא function חדש)** — כבר עשה 100% ממה שנדרש ל"אשר ושגר" (lookup+תרגום-מגודר-מחלקה+Whapi+`whapi_message_id`) עבור מקור אחר (`manual`/`inbox_routed`), ונקרא כבר היום מ-`WhatsAppInbox.js`'s `routeTask()` באותה תבנית בדיוק ("insert עכשיו, dispatch נפרד אח"כ") — הוחלט להרחיב במקום לשכפל. מקבל כעת `editedDescription`/`reviewerId` אופציונליים. **הליבה הקריטית:** אם השורה הנשלפת היא `status:"pending_approval"` — מבצע **guarded conditional UPDATE ראשון** (`.eq("status","pending_approval")` + `.select().maybeSingle()`) שמעביר ל-`open`+מחשב `sla_deadline` מ-`dispatched_at` דרך `buildGuestOpsSlaDeadline` הקיים (יובא מ-`_shared/automationSchedule.ts`) — **לפני** כל תרגום/שיגור. אם 0 שורות חזרו (מישהו אחר כבר טיפל) → `{ok:true, notified:false, reason:"already_processed"}`, יציאה נקייה בלי לגעת ב-Whapi בכלל. משימות `manual`/`inbox_routed` (כבר `open` מלכתחילה) מדלגות על הענף הזה — ללא שינוי התנהגות. `buildManualTaskCard`'s prefix קיבל ענף `source==='guest_request'` → `[GUEST WA]` (היה נופל בטעות ל-`[MANUAL TASK]`).
- ✅ **`OperationsBoard.js`** — טאב חדש `pending_approval` (ראשון ברשימה, pulse ויזואלי בהשאלה מ-`sla-breach-pulse` הקיים כש-count>0, default-select חד-פעמי בטעינה ראשונה בלבד — לא "גונב" טאב ממישהו שכבר צופה ב-"בוצעו" כשמגיעה בקשה חדשה ב-realtime). `TaskCard`: badge אמבר ייעודי + textarea לעריכת התיאור (seed חד-פעמי מ-`task.description`, לא מסתנכרן מחדש כדי לא לדרוס עריכה באמצע) + כפתורי «✅ אשר ושגר» (קורא ל-`notify-manual-task` המורחב) ו-«🗑️ דחה» (guarded UPDATE ישיר ל-`rejected`, סיבה אופציונלית טרם אישור סופי) — claim/done מוסתרים ב-state הזה. `applyTaskRowUpdate`'s רשימת ה-no-op הורחבה (`dispatched_at`/`reviewed_by`/`sla_deadline`/`rejection_reason`) כדי שהכרטיס יתעדכן חזותית מיד עם ה-realtime event.
- ⚠️ **באג אמיתי נמצא ותוקן תוך כדי audit** (לא קשור ישירות לפיצ'ר, אבל היה מתפוצץ ברגע הראשון שנוצרה שורת `pending_approval` חיה) — `src/App.js`'s `openTasks`/`urgentTasks` (Dashboard tile "🛠️ תפעול") ו-`openOpsCount` (בדג' ה-sidebar) השתמשו ב-`t.status !== "done"` (רשימת-החרגה) — היו סופרים `pending_approval` (עדיין לא ניתן לפעולה) וגם `rejected` (בוטל לצמיתות) כ"עבודה פתוחה". הוחלף לרשימת-הכללה מפורשת `t.status === "open" || t.status === "in_progress"` בשלוש הנקודות — עקרון FAIL VISIBLE §0.3 (רשימת-החרגה בולעת בשקט כל ערך status עתידי; רשימת-הכללה סופרת רק את מה שבאמת מיועד).
- ✅ **Scope מכוון, לא נגעתי (מאומת מול קוד, לא הנחה):** `admin_reception_tasks` (`handleAdministrativeInHouseIntercept`, ספא/קבלה) ו-Requests Board (`guest_alerts` — בלונים/upsell/הזמנות פורטל) — שניהם מעולם לא הגיעו לקבוצת הוואטסאפ הזרה ממילא, אז אין את הסיכון שהפיצ'ר הזה קיים כדי לגדר. `sla-escalation-cron`'s שאילתת ה-unassigned scan (`.eq("status","open")`) כבר מדירה `pending_approval` אוטומטית בלי שינוי קוד. RLS על `tasks` (migration 015) כבר `USING(true)` לכולם — לא נדרש שינוי.
- ✅ תהליך: audit קוד ישיר (לא הנחה) על `routeGuestRequestToOpsGroup`+3 נקודות הקריאה+`notify-manual-task`+`OperationsBoard.js`, ואז Plan Mode עם סוכן Plan מקביל לאימות/אתגור העיצוב — הסוכן טעה בטענה אחת (שאין self-test convention בריפו על widening; אומת ישירות מול migrations 100/139 שהטענה שגויה) ותיקנתי בפלאן הסופי; מצא נכון את באג ה-App.js. Mike אישר את הפלאן (כולל 4 ההחלטות למעלה) לפני שנכתב קוד.
- ✅ `npm run build` → `Compiled successfully.` (582.37kB +1.19kB, אפס warnings חדשים). `npm test` → 19/19 test suites, 154/154 tests.
- ⚠️ **לא deployed** — ממתין לאישור Mike: `npx supabase db push` (migration 149) + `npx supabase functions deploy whatsapp-webhook notify-manual-task --no-verify-jwt` + `git push` (src/ + supabase/functions שינויים). לא נבדק חי בדפדפן (Deno Edge Functions + realtime approval flow דורשים DB חי, לא ניתן לדמות ב-preview מקומי) — Mike צריך לשלוח הודעת בדיקה אמיתית ("אפשר לקבל עוד מגבות בבקשה") מאורח `checked_in` ולעבור על checklist האימות (§10 session 132 verification plan, ראה plan file).

#### session 127 — 2026-07-06 (DREAM BOT Inbox — full-history search + WhatsApp-style own-send scroll)
> הקשר: Mike דיווח שתי בעיות בשימוש בפועל ב-`WhatsAppInbox.js`: (1) חיפוש הרוסטר לא מוצא "הכל" — לא ברור אם כל השיחות/הודעות ניתנות לאיתור, (2) בכל שליחת הודעה תחושה שה"שיחה נעלמת" כי אי-אפשר לראות את מה שלמעלה — ביקש שזה יתנהג "כמו וואטסאפ": קודם רואים את כל השיחה, ורק אז ממשיכים.

- 🔍 **Audit לפני קוד (לא הנחה).** נקרא `WhatsAppInbox.js` במלואו (~5300 שורות): `fetchAll`/`fetchSince`/`fetchThreadHistory`/`mergeThreadRows`/`mergeIncomingRows`/`openContact`/הscroll effect. אושר: session 125c's additive-merge contract (`mergeThreadRows` — "never drops local rows") **כבר** מונע מחיקת היסטוריה בתוך פגישה חיה אחת — אין "איפוס" של thread על שליחה רגילה (`sendManualReply` קורא `fetchSince()`, לא `fetchAll()`/`fetchThreadHistory(force)`). שני פערים אמיתיים אותרו, לא השערה:
  - **(א) חיפוש היה מוגבל לזיכרון בלבד.** `rosterSearch` הקיים סינן רק `contacts` הנגזר מ-`allMsgsRef.current` — שמכיל את `fetchAll`'s חלון 400 השורות **האחרונות בכל האתר יחד** (לא per-guest) בתוספת כל thread שנפתח ידנית באותה פגישה. אורח שלא הוחלף עמו הודעה לאחרונה, או מילת-מפתח שהוזכרה רק בהודעה ישנה, לא היו ניתנים לאיתור כלל — "החיפוש" נראה מלא אך היה חלקי, בלי FAIL VISIBLE שמסמן את זה (§0.3).
  - **(ב) שליחה עצמית לא כפתה גלילה לתחתית.** `nearBottomRef` guard (session 125c — נועד למנוע קפיצה על עדכון *נכנס* בזמן קריאת היסטוריה) חסם גם את הגלילה להודעה *היוצאת* של הצוות עצמו: אם הצוות גלל למעלה לקרוא הקשר ואז שלח תשובה, ההודעה נחתה מתחת לגלילה בלי שום משוב חזותי — מרגיש כאילו "השיחה נעלמה", בניגוד לוואטסאפ אמיתי שתמיד גולל את השולח להודעה שלו-עצמו.
- ✅ **תיקון (א) — Full-inbox DB search.** `useEffect` חדש (debounced 400ms, מינ' 2 תווים) אחרי `refreshActiveThread`: שואל ישירות `whatsapp_conversations.message ILIKE %q%` (תוכן הודעה, לא רק מטא-דאטה של איש קשר) **וגם** `guests` name/phone/room ILIKE (אותה קונבנציית `.or(...)` כמו `NewChatModal`'s `guestSearch` הקיים) — עד 10 טלפונים חדשים ל-hydration בכל חיפוש (bounded, לא unbounded blast). תוצאות מאוחדות דרך `mergeThreadRows`/`fetchThreadHistory` **הקיימים ללא שינוי** (אותו `THREAD_HISTORY_LIMIT`, אותו additive-merge contract) — אורח שנמצא רק דרך ה-DB מקבל את מלוא ההיסטוריה שלו בדיוק כמו פתיחה ידנית, לא רק preview. `dbSearchBusy` state + label חדש `t.dbSearching` ("🔎 מחפש גם בהיסטוריה המלאה…"/"Searching full history too…") מוצג מתחת לשדה החיפוש (מובייל+דסקטופ) בזמן החיפוש. `searchPh` (HE+EN) עודכן לציין שהוא כולל תוכן הודעה, לא רק שם/טלפון/חדר.
- ✅ **תיקון (ב) — WhatsApp-style own-send scroll.** `sendManualReply` קובע `nearBottomRef.current = true` באופן יזום, **לפני** ה-network round-trip (לא אחריו) — כך שגם אם `fetchSince()`/Realtime יזרימו את ההודעה שנשלחה תוך כדי ההמתנה, ה-scroll effect (תלוי ב-`thread.length`) יגלול לתחתית ברגע שהיא נכנסת. שאר נקודות השליחה (`handleSendBulk`/`handleSendFree`/`handleSendTemplate*`) נבדקו ואינן רלוונטיות — כולן ב-`NewChatModal` לפתיחת שיחה חדשה, לא בתוך thread פתוח; `sendManualReply` היא נקודת השליחה היחידה בתוך שיחה פתוחה.
- ✅ `npm run build` → `Compiled successfully.` (575.54 kB +415B, אפס warnings חדשים). `npm test` → 16/16 test suites, 123/123 tests (כולל `WhatsAppInbox.test.js`'s `mergeThreadRows`/`groupByPhone` הקיימים, לא נגעתי בהם).
- ⚠️ **לא נבדק חי בדפדפן.** `preview_start` + ניסיון login דמו (`eliad`/`1234`) נדחה ("שם משתמש או סיסמה שגויים") — אותה מגבלה מתועדת ב-sessions 47/51/52 (Google OAuth client ID גם חסר בסביבת הפיתוח: `חסר REACT_APP_GOOGLE_CLIENT_ID`). קוד נקרא/נבדק בקומפילציה+טסטים בלבד — **Mike צריך לאמת עם login אמיתי:** (1) הקלדת שם אורח/מספר/מילת-מפתח מהודעה ישנה בחיפוש מוצאת שיחה שלא הייתה גלויה קודם, (2) גלילה למעלה בשיחה ארוכה ואז שליחת תשובה גוללת אוטומטית להודעה שנשלחה. לא deployed — ממתין לאישור לפי §12/§13 (רק `src/` השתנה, אין migration/function — צריך רק `git push`, אין `db push`/`functions deploy`).
...
#### session 126 — 2026-07-06 (Suite vs. day-guest automation audit — premium_day_guest classification + portal order sync)
> הקשר: Mike ביקש לוודא שגם אורחי סוויטה וגם אורחי-יום (day_guest + premium_day_guest — היום מיובאים בפועל רק "פרימיום") מקבלים אוטומציה נכונה, כל אחד עם התרחיש שלו, ושהפורטל שהבוט שואב ממנו מסונכרן נכון עם ה-DB לכל סוג אורח.

- ✅ **Audit לפני קוד — מנוע הכשירות/דיספאץ' עצמו תקין.** נקראו `_shared/automationSchedule.ts`, `_shared/suiteNames.ts`, `whatsapp-send`, `whatsapp-cron`, `guest-portal-data`, `guest-portal-order`, `ArrivalImportPanel.js`, `AddGuestModal.js` וה-migrations הרלוונטיות (065/093/094/096/097/099/132/134/135). `isEffectiveSuiteGuest`/`isEffectiveDayPassGuest` (session 125 P0) מסווגות נכון בכל מקום; `automation_stages` יש לו שלב day-pass מקביל לכל שלב סוויטה (`night_before_daypass`/`mid_stay_daypass`/`checkout_fb_daypass`, משותפים `pre_arrival_2d`/`stage_2_arrival`/`morning_welcome`), כל אחד עם `bot_scripts` מנוסח-יום-כיף נפרד. `day_guest` ו-`premium_day_guest` מטופלים **זהה לחלוטין** בכל מנוע הדיספאץ' (`DAY_PASS_ROOM_TYPES` ב-`suiteNames.ts`) — בכוונה, לא באג. הפער האמיתי היה במעלה הזרם — איך `room_type` נכתב ונשמר.
- ✅ **באג 1 — `sync_suite_arrivals` (migration 144, מחליפה 135).** ה-CASE של `v_room_type` הפיק רק `day_guest`/`suite`/`standard` — אף פעם לא `premium_day_guest`, למרות ש-migration 096 (session 57) הוסיפה אותו ל-CHECK constraint במפורש בשביל זה, ותיעדה רשימת consumers (`AddGuestModal`/`GuestDashboard`/`GuestPortal`/`WhatsAppInbox`/`BroadcastDashboard`/`AutomationControlCenter`) — בלי `sync_suite_arrivals` עצמה. וגם: `room_type` נכתב ללא-תנאי בכל re-sync (גם ה-UPDATE branch וגם `INSERT...ON CONFLICT DO UPDATE`) — כל אורח שסגל שידרג ידנית ל-`premium_day_guest` דרך `AddGuestModal.js` חזר בשקט ל-`day_guest` בפעם הבאה שאותו יום יובא/סונכרן מחדש (פעולה שגרתית), מה שדרס את הבאדג' ⭐ ב-`GuestDashboard` ואת סינון ה-`visibility_settings` בפורטל (migration 097) — האורח ראה תוכן יום-כיף גנרי במקום תוכן פרימיום. **תוקן:** לפני חישוב `v_room_type`, שליפת הערך הקיים (`v_existing_room_type`) ברגע ש-`v_guest_id` נפתר — אם הוא כבר `premium_day_guest` והדאטה החדשה עדיין אומרת "יום-כיף" (לא סוויטה), נשמר במקום להידרס. בנוסף: אחרי לולאת החדרים (שכותבת `guests.room`), UPDATE ממוקד חדש (מוגבל לתאריכי הגעה שה-sync הזה בפועל נגע בהם) מקדם `day_guest`→`premium_day_guest` כשהחדר שהוקצה הוא בדיוק `Premium Day 1`/`Premium Day 2` — אותו סיגנל בדיוק ש-`inferRoomType()` (`AddGuestModal.js:32-43`) כבר משתמש בו ידנית, מוחל אוטומטית לנתיב הייבוא (EZGO Suites CSV, ה-`rooms[]` payload) שכן נושא שם חדר. **הערה חשובה:** דוח ההזמנות המפורט (Detailed Reservation Report — המקור שבפועל בשימוש כיום, למשל `01.7.26.csv`) אין לו עמודת שם-חדר/חבילה בכלל (`detailedReservationParser.js`'s `DETAILED_REPORT_COL` — רק ספירת חדרים, לא שם) — לא ניתן לזהות "פרימיום" ממנו אוטומטית; התיקון לא ממציא זיהוי שאין לו סיגנל, רק מפסיק להרוס סיווג שכבר נקבע נכון (ידנית או דרך המסלול השני) ומיישם את הזיהוי הקיים איפה שהסיגנל כן זמין.
- ✅ **באג 2 — `guest-portal-order/index.ts`.** הפונקציה היחידה בכל `supabase/functions` שנשארה קוראת את `upsell_items.target_audience` הישן/קפוא (grep אישר: 0 מופעים נוספים שם וב-`src`) — migration 097 (session 57) העבירה את `guest-portal-data`+`PortalSettingsPanel.js` ל-`visibility_settings` בדיוק כי `target_audience` לא יכול לייצג `premium_day_guest` (097's ה-comment שלה מתעד את הבאג הזה כ"תוקן" — אבל `guest-portal-order` לא עודכנה מעולם). בפועל: `validAudiences` הוסיפה `"day_use"` (אוצר-מילים ישן מלפני 097, לא `"day_guest"`) עבור `day_guest`, ולא הוסיפה **כלום** ייעודי עבור `premium_day_guest` — כך שאורח `premium_day_guest` יכול היה להזמין רק פריטים עם `target_audience==='all'`; ופריט שסגל שינה את ה-`visibility_settings` שלו אחרי migration 097 (מבלי לגעת ב-`target_audience` הקפוא, ש-`PortalSettingsPanel.js` כבר לא כותב) עלול היה ליפול בשקט מהעגלה לכל סוגי האורחים — האורח רואה את הפריט בפורטל (`guest-portal-data` תקינה) אבל ההזמנה שלו נעלמת בלי שגיאה. **תוקן:** אותה בדיקת array-contains בדיוק כמו `guest-portal-data.ts:170-176` (`guestRoomType` לא ידוע→הכל מותר, `visibility_settings` ריק/חסר→הכל מותר, אחרת חייב לכלול את `room_type` של האורח) — ה-select הוחלף מ-`target_audience` ל-`visibility_settings`.
- ✅ **`supabase/tests/sync_suite_arrivals_premium_test.sql`** חדש — 5 תרחישי BEGIN/ROLLBACK (מראה את `voucher_reconciliation_e2e_test.sql`, session 54, לא inline self-test בתוך ה-migration עצמה — הפונקציה הזו כבר נדרשה הוטפיקס פעמיים, 134→135, מוטציה-ונקיון-מושלם בתוך migration transaction בדיוק סוג הדבר ששבר אותה בעבר): פרימיום ששורד re-sync (A), קידום אוטומטי משם-חדר (B), יום-כיף רגיל לא מקודם בטעות (C), סוויטה לא נפגעת (D), וסיווג-מחדש אמיתי לסוויטה מנצח תג-פרימיום ישן (E).
- ⚠️ **דגל ל-Mike, לא תוקן — צריך בדיקה שלו, לא קוד עיוור.** כמה תבניות Meta ל-fallback יום-כיף חולקות שם עם תבניות הסוויטה: `morning_welcome`'s יום-כיף נופל ל-`suite_welcome_morning`/`suite_welcome_morning_shabbat` (`whatsapp-send/index.ts:2469`); `mid_stay_daypass`/`checkout_fb_daypass`→אותו שם תבנית בדיוק כמו `mid_stay`/`checkout_fb` (`PIPELINE_TEMPLATE`, שם קובץ, שורות 213-217). זה קורה רק כשחלון 24ש' סגור וחובה תבנית מאושרת — המסלול השכיח (חלון פתוח) כבר משתמש ב-`bot_scripts` מנוסח-יום-כיף נכון (`morning_daypass`/`night_before_daypass`/`mid_stay_daypass`/`checkout_fb_daypass`). אין לי גישה לתוכן התבנית המאושרת בפועל מכאן — Mike צריך לבדוק ב-Meta Business Manager / "📋 ניהול תבניות" אם הניסוח כללי-מספיק לאורח-יום. גם לא אומת מכאן: מצב `is_active` חי של `night_before_daypass`/`mid_stay_daypass`/`checkout_fb_daypass`/`morning_welcome` ב-"מסע האורח" (Automation Control Center) — אין גישת קריאה ל-DB החי מהסביבה הזו.
- ⚠️ **לא deployed** — ממתין ל-`npx supabase db push` (migration 144) + `npx supabase functions deploy guest-portal-order --no-verify-jwt`, ולריצת `supabase/tests/sync_suite_arrivals_premium_test.sql` ב-SQL editor לפני אישור. `npm run build` נקי (אין שינויי `src/`).
...
#### session 97 — 2026-07-03 (Sensitive Stay-Change Shield — "room + early" phrasing gap → spa-topic leak)
> הקשר: Mike דיווח שהודעת אורח "היי רציתי לדעת אם יש אפשרות לקבל את החדר יותר מוקדם ?" (בקשת צ'ק-אין מוקדם) קיבלה תשובת ספא אוטומטית ("קיבלנו את הבקשה שלך לטיפול בספא..."). ההנחה הראשונית (של הבקשה) הייתה קלאסיפייר-ספא רחב מדי (regex/keyword) שתפס מילים כמו "חדר"/"מוקדם"/"אפשרות" — לא נמצא קלאסיפייר כזה בקוד בפועל.

- 🔍 **Audit — נבדק כל classifier רלוונטי מול הטקסט המדויק (Node, לא ניחוש):** `UPSELL_PATTERNS`, `COMPLAINT_PATTERNS`, `ADMINISTRATIVE_IN_HOUSE_PATTERN` (הספא-ספציפי, `automationSchedule.ts`), ו-`SENSITIVE_STAY_CHANGE_PATTERN` (המגן הייעודי ל-early-checkin/late-checkout/room-change, session 76b). כולם נבדקו ישירות מול `"היי רציתי לדעת אם יש אפשרות לקבל את החדר יותר מוקדם ?"` — **אף אחד לא התאים**, כולל המגן שאמור לתפוס בדיוק את זה.
- ✅ **הממצא האמיתי:** `SENSITIVE_STAY_CHANGE_PATTERN`'s תת-הביטויים ל"צ'ק-אין מוקדם"/"הגעה מוקדמת"/"כניסה מוקדמת" דרשו את המילה "מוקדם"/"מוקדמת" צמודה ישירות (`\s*`) למילת ההקשר — אבל הניסוח הטבעי של אורח ("לקבל את **החדר** יותר **מוקדם**") מפריד בין "חדר" ל"מוקדם" במילים אחרות ("יותר"). המגן הזה — שקיים בדיוק כדי למנוע מ-LLM לטפל בבקשות שינוי-שהות — פספס את הבקשה בגלל הפער הזה, וההודעה המשיכה ל-`classifyIntent()`'s ברירת המחדל ("faq") → branch ה-LLM החופשי. שם, `buildGuestStageContext()` (שורה 882) **תמיד** מזריק להקשר את שעת טיפול הספא של האורח כשקיימת (רוב האורחים בריזורט מזמינים ספא) — סביר שזה מה שהטה את תשובת המודל לספא במקום להתייחס לבקשת החדר בפועל. אותה בעיית-הצמדה (`\s*` ולא tolerant-gap) נמצאה גם ב-"לצאת...מאוחר" ("לצאת מהחדר מאוחר יותר" נכשל מאותה סיבה) — תוקן באותו מעבר כי זו אותה מחלקת בעיה בדיוק ("early/late" phrasing, מה שהתבקש במפורש).
- ✅ **התיקון (`_shared/automationSchedule.ts`, `SENSITIVE_STAY_CHANGE_PATTERN`):** שני האלטרנטיבות עברו מ-adjacency-only (`\s*`) ל-gap-tolerant (`[\s\S]{0,20}`/`[\s\S]{0,25}`) — "חדר[\s\S]{0,25}מוקדם"/"מוקדם[\s\S]{0,25}חדר"/"להיכנס\s*(לחדר\s*)?מוקדם" ו-"לצ.?את[\s\S]{0,20}מאוחר" — כך שמילים מפרידות ("יותר", "את ה-") לא שוברות את ההתאמה. **נבדק מול 13 מקרי בוחן** (Node, ישירות מהקובץ הערוך): ההודעה המדווחת + וריאציות דומות (True), וגם מקרים שחייבים **להישאר** לא-חסומים — שאלת FAQ תמימה על שעת הכניסה ("מה השעה של הכניסה לחדר?"), שאלת ספא אמיתית ("מה שעת הטיפול שלי בספא"), תלונת מזגן ("בעיה בחדר, המזגן לא עובד"), ובקשת מגבת ("עוד מגבת בחדר") — כולם נשארו `false` כנדרש, כך שהגמישות החדשה לא הרחיבה את הרשת יתר על המידה לכיוונים אחרים.
- ✅ **מסלול הטיפול לא השתנה** — ברגע שהבקשה נתפסת ב-Tier-0 (pre-burst, לפני ה-LLM, אפס עלות טוקנים — `whatsapp-webhook/index.ts:3502`), `handleSensitiveStayChangeHandoff` שולח את `CANONICAL_STAY_CHANGE_HANDOFF_MSG` הקבוע ("העברתי את בקשתך לצוות הסוויטות שלנו, והם יצרו איתך קשר בהקדם") + כותב `requires_attention=true`/`needs_callback=true`/`attention_reason="date_change"` — אותה קונבנציה בדיוק שכבר קיימת לכל בקשת שינוי-שהות אחרת (מוצג ב-`GuestAttentionBadge.js` כ-🗓️). לא נוצר reason חדש, לא נדרש שינוי ל-`GuestAttentionBadge.js`/UI. **לא חובר** `staff_color_label` (migration 123, session 94) — זהו שדה תיוג-ידני-בלבד לצוות (Guest Context Drawer), לא חלק מהמנגנון האוטומטי הקיים; שימוש בו כאן היה מכניס כתיבה אוטומטית לשדה שתוכנן להיות ידני-בלבד, בניגוד לקונבנציה התיעודית הקיימת.
- ⚠️ **גבול-scope מכוון:** לא נגעתי ב-`buildGuestStageContext()`'s הזרקת שעת-ספא הגורפת (שורה 882) — זו התנהגות רחבה יותר (הזרקת הקשר ל-LLM לכל שיחת FAQ, לא רק ל-early-checkin) ומחוץ לתחום הבקשה הנוכחית; מתועד כאן כתצפית-משנה, לא טופל בסבב הזה. אם ישנן בעתיד תלונות דומות על "המודל ענה על נושא לא-קשור" בהקשרי FAQ אחרים (לא רק early-checkin, שכעת חסום ב-Tier-0 ולעולם לא מגיע ל-LLM) — כדאי לבדוק את הזרקת ה-context הזו בנפרד.
- ✅ `npm run build` נקי (שינוי יחיד ב-`supabase/functions/_shared/automationSchedule.ts` — regex בלבד, אין שינוי `src/`). **⚠️ Deno CLI לא זמין בסביבת הפיתוח — לא הורץ `deno check` מקומי.** `_shared/automationSchedule.ts` נארז לתוך **כל** function שמייבא אותו (whatsapp-webhook + whatsapp-cron + automation-queue + guest-portal-ops-request + sla-escalation-cron ועוד, ראה §12 warning) — נדרש redeploy לכולם לפני שהתיקון חי, לא רק ל-whatsapp-webhook.

#### session 98 — 2026-07-03 (Guest Import Intelligence — Sprint 3: sync wiring + DB match)
> הקשר: Sprint 0–2 (session 92 area) built `mergeCandidates`/`classifyDbMatch` (`src/utils/guestImportIntelligence.js`) and wired a **read-only display badge** into `ArrivalImportPanel.js`'s grid — but only ever called `classifyDbMatch(c, null)`, so the badge could only ever say "⛔ מטריית קבוצה" or nothing. `handleSync()` itself still read every field straight off the raw per-source profile (`merged[i]`), never off the merged candidate. This sprint wires the merge layer into the real sync path end-to-end.

- ✅ **`_getSyncProfileIndices`** (now `export`ed for testability) takes a new `dbMatchByIdx` param and returns `{ indices, conflicts, skippedUnimportable }` instead of a bare array — `unimportable` rows are skipped (Disable Don't Hide §0.2: still visible in the grid with their badge, just never reach the RPC), `conflict` rows are **not** skipped (they sync like any match, just get flagged for review — FAIL VISIBLE §0.3), rows with no `guestPhone` are skipped as before.
- ✅ **DB prefetch (`existingGuestsMap`)** — a `useEffect` keyed on `mergedCandidates` batch-fetches `guests(id, phone, name, room, order_number, arrival_date)` for every distinct `arrivalDate` present in the session's candidates (one query, not one per row), keyed by `phone::arrival_date` **and** `order_number::arrival_date` (`_findExistingGuestRow`). Scoped by arrival_date on both keys deliberately — `guests` has no global phone uniqueness (migration 046: phone+arrival_date+guest_index), so an unscoped phone-only Map would false-positive a returning guest's prior stay as "existing" for a new one.
- ✅ **`dbMatchByIdx`** (new memo) runs `classifyDbMatch(candidate, existingRow)` per candidate against the prefetched Map — real `new`/`existing`/`conflict`/`unimportable` classification, not just unimportable-or-blank. `importBadgeByIdx` now maps all four statuses to labels (🆕 חדש / 🔄 קיים / ⚠ התנגשות / ⛔ מטריית קבוצה) via `DB_MATCH_BADGE_LABEL` — visible live in the grid as the file loads, before the sync button is even clicked.
- ✅ **`handleSync()` PATH A** — the `profiles`/`rooms` payload builders and the post-RPC enrichment patch loop (spa_time/meal_time/meal_location/guest_notes → `guests`, room_count → `bookings`) now read `guestPhone`/`guestName`/`arrivalDate`/`orderNumber`/`treatment_count`/`leadSource`/`automationMuted`/`nights`/`price`/`spa_time`/`meal_time`/`meal_location` off `mergedCandidates[i]` first, falling back to the raw `merged[i]` profile only for fields the candidate model doesn't carry (the granular per-room breakdown — `g.rooms`/`resLineId`/`coordPhone` — stays arrivals-only, per `FIELD_SOURCE_PRIORITY.room`). Grid edits (`edited.*`) still win over both when the user manually typed something.
- ✅ **Result panel + toast** — `result.skippedUnimportable`/`result.conflictCount`/`result.conflictNames` render as new warning lines under the existing `result.skippedRooms` line (same amber FAIL-VISIBLE convention already used there) instead of a transient toast — persistent, not a 4.5s flash. The "no records to sync" toast now distinguishes "everything in this filter was unimportable" from the generic empty-filter message.
- ⚠️ **Scoped out (per the task's own instruction):** passing a simultaneous `detailed` array alongside `arrivals` into `mergeCandidates` at sync time — the codebase's `doc2Map` state slot is shared between the "suites" and "detailed" import modes (`importSource` selects which), so there is currently no session state where both raw sources coexist to merge together. `mergedCandidates` already does the correct `detailed: OR arrivals: (+ ops: always)` call exactly as the display layer did.
- ⚠️ **Known caveat for QA, not fixed this sprint:** `classifyDbMatch`'s `roomConflict` check compares `candidate.room` (raw CSV `roomName`/`suiteType` text, per `FIELD_SOURCE_PRIORITY.room=["arrivals"]`) against `existingGuestRow.room` (the SUITE_REGISTRY-formatted display value actually stored in `guests.room`) — these two strings can differ in format even when nothing really changed (e.g. raw "Suite A1" vs. stored "וילה 1"), which may over-trigger "conflict" badges on repeat imports of the same file. Non-blocking (conflict rows still sync), but worth watching in Mike's live test.
- ✅ **Tests** — `guestImportIntelligence.test.js` gained 4 new cases: `classifyDbMatch` with a real (non-null) `existingGuestRow` → `existing` (identical name/room/date), `conflict` (name mismatch), `conflict` (room mismatch via order_number join), `new` (no phone/order overlap). New `src/components/ArrivalImportPanel.test.js` (5 cases) covers `_getSyncProfileIndices`: skips `unimportable`, still skips no-phone rows regardless of DB status, does **not** skip `conflict` rows (reports them separately), respects `selectedIds` on top of DB gating, and never throws when `dbMatchByIdx` is omitted. `npm run build` clean, `npm test` 28/28 passing (4 suites).
- ⚠️ **Not deployed** — frontend-only (`src/`), no `supabase/` changes. Awaiting Mike's `תעלה` before `git push`.

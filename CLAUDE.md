...
#### Tier-0 Operational In-House Intercept (session 76)
```
אורח ב-status checked_in + מילת מפתח תפעולית (חלב|מים|מגבות|ניקיון|מזגן|…):
  ✅ אחרי dedup wa_message_id, לפני burst/LLM — אפס טוקנים
  ✅ routeGuestRequestToOpsGroup → tasks (source=guest_request, status=open, guest_id)
  ✅ guests.requires_attention=true + attention_reason="בקשת X לחדר"
  ✅ תשובה דטרמיניסטית buildOperationalDispatchReply — בלי "אני בודק מול דלפק"
מקור: whatsapp-webhook handleOperationalInHouseIntercept + automationSchedule.ts
```

#### Sensitive Stay-Change Shield (session 76b)
```
הארכת שהייה / late checkout / צק-אין מוקדם / שינוי חדר:
  ✅ intercept לפני upsell ו-LLM — אסור "בשמחה רבה" או אישור משתמע
  ✅ תשובה קנונית בלבד: CANONICAL_STAY_CHANGE_HANDOFF_MSG
  ✅ needs_callback=true + requires_attention + attention_reason='date_change'
  ✅ pre_send_guard — אם LLM דלף בכל זאת, דורס לפני שליחה
  ❌ שאלות מידע ("מתי צק אאוט?") — לא נתפסות (FAQ exclusion)

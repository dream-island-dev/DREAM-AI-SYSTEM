...
- orit_cs_agent ➔ OritCustomerServicePanel.js (סוכן שירות לקוחות לאורית — **Graph API read-only** לתיבת M365 שמקבלת Forward מ-`dream-island.co.il`, טעינה דרך `orit-cs-bootstrap` (service-role, עוקף RLS), כפתורי «חברי Outlook 365» + «סנכרן עכשיו», AI סיכום+טיוטות, העתקה ידנית ל-Outlook, דייג'סט בוקר Whapi). אין שליחה מהמערכת.

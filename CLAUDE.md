...
- ✅ `npm run build` נקי (רק האזהרה הקיימת) + `npx supabase db push` (migration 057) + `npx supabase functions deploy whatsapp-webhook --no-verify-jwt` — backend פרוס. ⚠️ **שינויי frontend (GuestDashboard/GuestsPage/WhatsAppInbox/GuestAttentionBadge/guestStatusMeta) עדיין לא committed/pushed** — לא יגיעו ל-Vercel עד commit+push מפורש.

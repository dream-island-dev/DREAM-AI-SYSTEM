...
1.1 Guest Experience Survey + Spa warm-up + Guest Club opt-in (MVP local 2026-07-13)

MVP אורח: בילוי יומי + ספא → `guest_surveys` + אוטומציות Option C. **מועדון לקוחות (Approach A):** אחרי שליחת סקר בפורטל — הצטרפות/סירוב → `guest_club_members` + `guests.club_status` (`guest-portal-club`). שידור הצעות (Whapi/Meta) = שלב הבא. Feedback → סקרים מציג חברים פעילים.

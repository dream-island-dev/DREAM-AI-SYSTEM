...
1.1 Guest Experience Survey + Spa warm-up (backend DEPLOYED; Meta survey-button template wired locally 2026-07-13)

MVP אורח: בילוי יומי (`day_guest`/`premium_day_guest`) **עם** ספא באותו יום (`guests.spa_date`=`arrival_date`) — `guest_surveys` (migration 194, סולם קטגוריות הורחב 1-5→1-10 ב-migration 196, 2026-07-13): 6 קטגוריות 1-10 + חוויה כללית 1-10 + טקסט חופשי, UNIQUE(guest_id, visit_date). `checkout_fb_daypass` מדולג לקבוצת ספא-יום (`superseded_by_survey`) — הם מקבלים את הסקר במקום, לא כפול. שני שלבי אוטומציה חדשים ב-`automation_stages`: `spa_warmup_daypass` (spa_time פחות 75 דק' — `anchor_event='spa_time'` חדש ב-`automationSchedule.ts`, משלב `guests.spa_date`+`spa_time`) ו-`survey_invite_daypass` (17:00 ביום ההגעה). **הזמנת סקר בוואטסאפ:** תבנית Meta `dream_survey_invite` (גוף Mike-lock + כפתור URL «למילוי הסקר» → portal/#survey) — wiring מקומי migration 199; חייב APPROVED ב-Meta לפני שיגור; ערוץ יום-כיף ACC עדיין Off כברירת מחדל. Whapi = טקסט+לינק בלבד (אין כפתור URL). דרך `guest-portal-survey` (Edge Function חדשה): ציון גבוה (overall≥8 וממוצע קטגוריות≥8.0 אחרי סולם 1-10) → קריאה לביקורת גוגל + CTA לסוויטות; ציון נמוך → שורת מראה ב-`guest_feedback` (`source='structured_survey'`) לתשומת לב צוות. `GuestPortal.js`'s `SurveySection` מוגן ע"י `guest.survey_eligible` השרתי (לא לוגיקת קליינט) — ניתן להרחיב לסוויטות בעתיד בלי שינוי UI. `WhatsAppInbox.js`: קבוצת בילוי-יומי+ספא מופרדת מברירת-המחדל ללשונית «☀️ ספא יום» נפרדת (כמו «אחרי עזיבה») כדי שלא תדלל את נקודות האדום של הסוויטות. `GuestFeedbackTabs.js`: תת-לשונית «📊 סקרים» (תצוגה מקדימה + עורך תוויות). `resort-digest-cron` כולל סעיף סקרים אופציונלי.

...
1.1 Guest Experience Survey + Spa warm-up (backend DEPLOYED; Option C window-opener wired 2026-07-13, await deploy)

MVP אורח: בילוי יומי (`day_guest`/`premium_day_guest`) **עם** ספא באותו יום (`guests.spa_date`=`arrival_date`) — `guest_surveys` (migration 194, סולם 1-10 ב-196). `checkout_fb_daypass` מדולג לקבוצת ספא-יום (`superseded_by_survey`). שלבים: `spa_warmup_daypass` (spa_time−75m) + `survey_invite_daypass` (17:00). **Option C (2026-07-13):** ערב/בוקר עם CTA «מחכים לכם!» לפתיחת חלון 24ש' ב-Meta; Whapi ראשי ליום-כיף עם failover אוטומטי ל-Dream Bot (session אם חלון פתוח, אחרת תבנית); תבניות גיבוי `dream_daypass_eve` / `dream_spa_warmup` / `dream_survey_invite` (PENDING עד Meta APPROVED). Session-first לשלבי יום-כיף כש-`wa_window` פתוח. דרך `guest-portal-survey`: ציון גבוה → Google + CTA סוויטות; נמוך → `guest_feedback`. Inbox «☀️ ספא יום»; Feedback «📊 סקרים».

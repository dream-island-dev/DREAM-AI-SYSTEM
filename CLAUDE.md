...
> **עדכון אחרון:** 2026-06-25 (session 44 — Zero-Rejection, Room Masking, OnceHub Removal & EasyGo Meal
> Parsing: Stage 2 (`stage_2_arrival`) — הוסר ה-OnceHub הסטטי, `{{portal_url}}` (alias ל-`{{PORTAL_LINK}}`
> הקיים) הוא הקישור היחיד עכשיו, כולל בתוכן ה-DB החי. **קיר ה"דחייה הקרה" (session 30 Sprint 5.5) הוסר
> לחלוטין** — בקשת אורח-עתידי (suite/standard, לא day_guest, לא checked_in) מנותבת בחן ל-`guest_alerts`
> עם תג `🟡 הגעה עתידית: [תאריך]` + DM ישיר לאדיר + תשובה יוקרתית לאורח (לא עוד "פתיחת בקשות שירות
> זמינה אחרי צ'ק-אין"). **Room Masking** — שם/מספר הסוויטה הספציפי מוסתר מכל פני-שטח מול-אורח לפני
> checked_in (הקשר ה-AI ב-webhook + `guest-portal-data`/`GuestPortal.js`) — חדר `dream_room_ready`
> (מסירת מפתח) הוא **חריג מכוון**, שם הגילוי קורה בכוונה. כפתור "בילוי יומי פרימיום" בפורטל הוחלף
> מ-LINK סטטי ל-REQUEST (Requests Board + DM מיידי לאדיר — לא עוד רק SLA אחרי 10 דק'). תג תאריך-עתידי
> אוחד לפורמט אחיד ("⚠️ בקשה עתידית לתאריך X - בעוד Y ימים") בכל שלוש נקודות היצירה. `AddGuestModal`
> ממלא "מסעדת ערמונים" כברירת מחדל לאורח חדש. ייבוא EasyGo מנסה לחלץ שעת ארוחה (⚠️ **לא אומת מול קובץ
> אמיתי** — regex ניחוש מיודע, ראה §10 session 44). ראה §10 session 44 לפירוט מלא.).
>
> **עדכון קודם:** 2026-06-25 (session 43 — Stage 2.5 Sabbath Logic, Check-In Profile Drawer & Restricted
> Cleaner RLS: `night_before` שונה ל-"Stage 2.5 — תזכורת ערב לפני 📅" (תצוגה בלבד, `stage_key` ללא שינוי).
> תוכן מדויק חדש נשמר ב-`bot_scripts.night_before_reminder` עם `{{entry_time}}`/`{{check_in_time}}` —
> מחושבים דינמית ב-`whatsapp-send`'s `resolveNightBeforeTimes()` מול `guests.arrival_date`: יום חול קבוע
> 12:00/15:00 (`bot_config`), שבת/חג נשלף מ-`bot_config` (`night_before_*_shabbat` — אושרו ע"י Mike
> בהמשך הסשן: כניסה 12:00, קבלת חדרים 18:00). FAIL VISIBLE עדיין פעיל כרשת-ביטחון: שליחה ביום
> שבת/חג עם ערך ריק (אם יתרוקן בעתיד) תיכשל בגלוי, לא תשלח שעה שגויה לאורח. תמונה
> (`image_3cde8f.jpg`) **לא קיימת בריפו** — נדרשת מ-Mike. `register-templates`'s
> `dream_checkin_reminder_v2` עודכן בקוד לתוכן החדש (**לא** נשלח ל-Meta עדיין). `GuestsPage.js` קיבל את
> אותה לחיצה-לפתיחת-`CustomerProfilePane` שכבר קיימת ב-`GuestDashboard.js` (קליק, לא hover — ראה הסבר
> §10 session 43). תפקיד `cleaner` ננעל אמיתית ברמת RLS (migration 087, RESTRICTIVE policies על 19
> טבלאות) — קודם לכן כל תפקיד מחובר (כולל cleaner) יכל לקרוא/לכתוב כל טבלה כמעט; `HousekeepingTabletView.js`
> עודכן להפסיק לקרוא מ-`guests` (היה מציג שם אורח — PII). ראה §10 session 43 לפירוט מלא.).
>
> **עדכון קודם:** 2026-06-25 (session 42 — Automation Recovery Audit & Guest-Timing Badges: אומת חי
> ש-`CRON_ENABLED=false` (תור האוטומציה "תקוע" אמיתי) **וגם** ש-2 מ-5 התבניות שה-cron התקופתי תלוי
> בהן (`dream_checkin_reminder_v2`/`dream_welcome_morning`) עדיין **PENDING** ב-Meta, לא APPROVED —
> הפעלה גורפת תיכשל באופן חזוי לשני שלבים מתוך חמישה. `CRON_ENABLED` **לא** הופעל בסשן זה — מחכה
> להחלטת Mike, ראה §10 session 42. נוסף `{{PORTAL_LINK}}` ל-Stage 2 (`stage_2_arrival`) — לא היה קיים
> בכלל קודם לכן (`portal_token` לא נטען, אין placeholder ב-`resolvePlaceholders()`). נוסף badge חי
> "🟡 הגעה עתידית"/"🟢 אורח בריזורט" ל-`OperationsBoard.js`+`RequestsBoard.js` (helper משותף חדש
> `src/utils/guestTiming.js`) + שורת הקשר לאדיר ב-2 נקודות שליחה (`guest-portal-ops-request`,
> `sla-escalation-cron`). ראה §10 session 42 לפירוט מלא.).
>
> **עדכון קודם:** 2026-06-25 (session 41 — Culinary UX Split & Strict Enterprise Routing: סצנת `wine.jpg` הפרידה מתוכן מסעדת ערמונים שהיה ממוזג בה בטעות — סצנה חדשה `armonim.jpg` קיבלה את התוכן + 2 כפתורים. סוג CTA שלישי, `OPS_REQUEST`, נוסף לצד `REQUEST`/`LINK` הקיימים — מנתב ללוח התפעול (`tasks`, migration 085, source חדש `portal_room_service`) + Edge Function חדש `guest-portal-ops-request` ששולח התראת WhatsApp ישירה לאדיר (לא לקבוצה — ⚠️ resolution דרך הלוח באפליקציה, לא 👍🏼, ראה §10 session 41 להסבר). ראה §10 session 41 לפירוט מלא.).
>
> **עדכון קודם:** 2026-06-25 (session 40 — Dynamic CMS, Vercel Asset Fix & Luxury UX Flow + Final Polish: `portalContent.js` הסטטי הוחלף ב-DB חי — טבלה חדשה `portal_scenes` (migration 084, RLS: קריאה ציבורית, כתיבה admin-only) + פאנל אדמין חדש "🎨 הגדרות פורטל" לעריכת סצנות בלי deploy. הורחב ל-7 סצנות, תוכן נשלף מ-dream-island.co.il (לא הומצא). ⚠️ שני באגי קבצים אמיתיים נמצאו ותוקנו (קובץ שהוחלף בשקט, שמות עם רווחים שמפילים CSS `url()` לא-מצוטט). 🎉 **פריצת דרך:** קיר ה-Auth שחסם אימות חזותי ב-sessions 19–40 **נפתר** — אובחן שורש (MOCK_USERS הוא dead code כש-Supabase מוגדר, שלא משפיע על סטאף אמיתי) ובוצעה התחברות אמיתית ומאומתת-בצילום-מסך עם חשבון QA (`claude-qa@dreamisland.internal`, role=admin, סיסמה **לא** בקובץ הזה — ראה §10 session 40 להוראות איפוס). פירוט מלא ב-§10 session 40 + התוספת בסופו).
>

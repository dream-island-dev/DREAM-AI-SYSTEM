...
- ⚠️ **Deno CLI לא זמין בסביבת הפיתוח** — לא הורץ `deno check` מקומי; `npm run build` לא רלוונטי (אין שינוי ב-`src/`). נדרש `supabase functions deploy whapi-webhook sla-escalation-cron task-action --no-verify-jwt` לפני שהשינוי חי — ראה §12.

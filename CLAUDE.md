...
- guests (Golden Profile):
  ...
  * **Smart Paste (migration 207, DEPLOYED):** `pg_trgm` GIN על `name` + RPC `match_guest_fuzzy`; Edge Function `parse-raw-paste` (Gemini 2.5 Flash, `suite_guest`|`day_guest`); UI `SmartPastePanel` ב-DataSyncPage — enrich / התראת חוסר / יצירת day guest.

-- Migration 125: sync_suite_arrivals — scope guests.room UPDATE to arrival_date
-- Bug: roomDisplay write used WHERE phone = v_phone only, overwriting room on
-- other stays for the same guest. Frontend also patches room post-RPC (session 99b).

-- 144_sync_suite_arrivals_premium_day_guest.sql
--
-- Fixes two related gaps in sync_suite_arrivals' room_type handling (live
-- version was migration 135 — this supersedes it, same signature, same
-- CREATE OR REPLACE convention as every prior hotfix to this function):
--
-- 1. v_room_type's CASE has never produced 'premium_day_guest' since migration
--    096 added it to the guests.room_type CHECK constraint — only 'day_guest'/
--    'suite'/'standard'. Worse, room_type was UNCONDITIONALLY overwritten on
--    every re-sync (both the UPDATE branch and INSERT...ON CONFLICT DO
--    UPDATE), so a guest a staff member manually promoted to
--    'premium_day_guest' via AddGuestModal.js got silently reverted to
--    'day_guest' the next time that day's arrivals were re-imported — wiping
--    out GuestDashboard's Premium badge and, more importantly,
--    guest-portal-data's visibility_settings-based scene/upsell segmentation
--    (migration 097) for that guest.
--
--    Fix: before recomputing room_type, look up the row's CURRENT value (once
--    v_guest_id is resolved). If it's already 'premium_day_guest' and the
--    fresh import data still says "day guest" (not suite), keep it.
--
-- 2. The Detailed Reservation Report import (no room/package-name column —
--    can never signal "premium") is the only source currently in regular use,
--    but the EZGO Suites import path DOES carry a real room/suite-type text
--    per row, from which _bestGuessSuite() can resolve "Premium Day 1"/
--    "Premium Day 2" as the assigned room (ArrivalImportPanel.js) — yet that
--    signal was thrown away; room_type only ever came from the isDayGuest/
--    hasSuite booleans computed before any room was assigned.
--
--    Fix: after the existing rooms loop sets guests.room, add one narrowly
--    scoped promotion pass — any guest THIS sync call touched whose room_type
--    is still 'day_guest' and whose assigned room is exactly 'Premium Day 1'
--    or 'Premium Day 2' gets promoted to 'premium_day_guest'. Mirrors
--    AddGuestModal.js's inferRoomType() (same two literal values), applied
--    automatically instead of requiring a manual follow-up edit every time.
--
-- Nothing else in this function changes — it has already needed two hotfixes
-- recently (134 → 135), so the diff here is deliberately minimal: one new
-- lookup + one new CASE branch + one new scoped UPDATE at the very end.
...
BEGIN
  ...
  FOR p IN SELECT * FROM jsonb_array_elements(COALESCE(payload->'profiles', '[]'::jsonb)) LOOP
    ...
    -- Premium-day-guest preservation (migration 144): look up the row's
    -- CURRENT room_type — if it's already 'premium_day_guest' and this
    -- import's own signal still says "day guest" (not suite), keep it rather
    -- than downgrading. Only covers the primary v_guest_id-resolved path
    -- above (the common case); the rare INSERT...ON CONFLICT collision
    -- fallback below is unchanged from before (same pre-existing limitation
    -- as every other field in that ON CONFLICT clause).
  ...
  -- Premium Day auto-promotion (migration 144) — see header comment #2.
  -- Scoped to arrival dates this call actually touched, never a full-table
  -- scan of unrelated historical guests.

-- 190_sync_suite_arrivals_null_phone.sql
-- Sprint C DOCS2: guests without a phone number are no longer silently
-- dropped on import (Zero Data Loss, CLAUDE.md §0.1). A row with a name or
-- an order_number now creates/updates a guests row with phone=NULL instead
-- of being skipped — only a row with NO phone AND NO name AND NO order is
-- still truly unimportable.
--
-- Key fixes vs 154_guest_automation_scope.sql:
--   1. Skip guard relaxed: phone OR name OR order is enough to import.
--   2. No-phone guest is forced automation_scope='muted' (cannot WhatsApp —
--      courtesy_only would still fire a Stage 4 send).
--   3. Tier-1/guest_index lookups made NULL-safe (phone IS NOT DISTINCT FROM)
--      so a repeat import of the same no-phone guest updates the same row
--      instead of duplicating it.
--   4. Tier for multi-room-per-order occupants gets an explicit name-scoped
--      branch for the no-phone case, instead of a blind NULL-safe match —
--      avoids merging two different unnamed occupants under one order.
--   5. Full (non-enrich) UPDATE no longer does `phone = v_phone` unconditionally
--      — COALESCE(v_phone, phone) so a blank-phone re-import can never null
--      out a phone staff already entered (Zero Data Loss).
--   6. public.bookings.phone is NOT NULL (031_treatment_time.sql) — the
--      bookings insert/update block is now guarded behind `v_phone IS NOT
--      NULL`. Without this guard, importing one no-phone guest would throw
--      inside the function and roll back the ENTIRE sync batch.
--   7. Returns createdWithoutPhone count for the FAIL VISIBLE frontend toast.
...
DECLARE
  ...
  v_lead_source TEXT; v_auto_muted BOOL; v_auto_scope TEXT; v_phone_strip TEXT;
  v_room_arrival DATE;
  v_guest_id BIGINT;
...
BEGIN
  ...
  FOR p IN SELECT * FROM jsonb_array_elements(COALESCE(payload->'profiles', '[]'::jsonb)) LOOP
    ...
    v_auto_scope := NULLIF(TRIM(p->>'automationScope'), '');
    IF v_auto_scope IS NULL OR v_auto_scope NOT IN ('full', 'courtesy_only', 'muted') THEN
      v_auto_muted := COALESCE((p->>'automationMuted')::BOOL, FALSE);
      v_auto_scope := CASE WHEN v_auto_muted THEN 'muted' ELSE 'full' END;
    ELSE
      v_auto_muted := (v_auto_scope = 'muted');
    END IF;
    v_old_phone := NULL;
    v_old_name := NULL;

    -- Cannot WhatsApp without a phone — force the most restrictive scope
    -- regardless of what the import payload asked for (defense in depth;
    -- the frontend already forces this too, see _parseGridAutomationScope).
    IF v_phone IS NULL THEN
      v_auto_scope := 'muted';
      v_auto_muted := TRUE;
    END IF;

    -- Truly unimportable: nothing to key a guest row off at all.
    IF v_phone IS NULL AND v_name IS NULL AND v_order IS NULL THEN
      RAISE NOTICE '[sync_suite_arrivals] SKIP guest — no phone, no name, no order';
    ...
    -- Tier 1: exact prior match (order+date+phone) — NULL-safe so a repeat
    -- import of the same no-phone guest re-finds itself instead of Tier-2
    -- re-deriving it (harmless either way, but this is the direct hit).
    ...
    -- Tier 2: single room under this order+date — already NULL-tolerant
    -- (only compares old vs new phone when old phone is non-blank), unchanged.
    ...
    IF v_guest_id IS NULL AND v_date IS NOT NULL THEN
      IF v_order IS NOT NULL THEN
        ...
        ELSIF v_name IS NOT NULL THEN
          -- Multi-room order, no phone this time — scope by name too so two
          -- different unnamed/no-phone occupants under one order never merge.
          SELECT id, guest_index INTO v_guest_id, v_guest_index
          FROM public.guests
          WHERE phone IS NULL
            AND arrival_date = v_date
      ...
      -- No-order fallback stays phone-strict on purpose: with neither phone
      -- nor order to key off, a NULL-safe match here risks merging two
      -- unrelated no-phone guests on the same date. Each becomes its own
      -- INSERT below instead (accepted duplicate-risk edge case).
    ...
    IF v_guest_id IS NULL AND v_date IS NOT NULL THEN
      ...
      FROM public.guests
      WHERE phone IS NOT DISTINCT FROM v_phone AND arrival_date = v_date;
    END IF;
    ...
    IF v_guest_id IS NOT NULL THEN
      ...
      IF v_enrich_only THEN
        UPDATE public.guests SET
          ...
          automation_scope  = public.merge_automation_scope(automation_scope, v_auto_scope),
          automation_muted  = (public.merge_automation_scope(automation_scope, v_auto_scope) = 'muted')
        WHERE id = v_guest_id;
      ELSE
        UPDATE public.guests SET
          -- Zero Data Loss: a blank-phone re-import must never erase a phone
          -- staff already entered — only ever set it when this import HAS one.
    ...
    ELSE
      ...
      ON CONFLICT (phone, arrival_date, guest_index) DO UPDATE SET
        ...
        automation_scope = public.merge_automation_scope(guests.automation_scope, EXCLUDED.automation_scope),
        automation_muted = (public.merge_automation_scope(guests.automation_scope, EXCLUDED.automation_scope) = 'muted');
    END IF;

    guest_count := guest_count + 1;
    IF v_phone IS NULL THEN
      no_phone_count := no_phone_count + 1;
      RAISE NOTICE '[sync_suite_arrivals] no-phone guest synced — name=%, order=%', v_name, v_order;
    END IF;

    -- public.bookings.phone is NOT NULL — a no-phone guest has no bookings
    -- counterpart (nothing for the Meta webhook to key off anyway). Guard
    -- the whole block so one no-phone row never throws and rolls back the
    -- rest of the batch.
    IF v_phone IS NOT NULL THEN
  ...
  RETURN jsonb_build_object(
    ...
    'createdWithoutPhone', no_phone_count,

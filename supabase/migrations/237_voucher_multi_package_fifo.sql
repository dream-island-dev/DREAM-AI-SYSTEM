-- ============================================================================
-- migration 237: Same ת.ז. + multiple EZGO vouchers (multi-package order) —
-- assign one EZGO row per provider redemption line (FIFO by package), not
-- duplicate_match. Also disambiguate via CouponNo in raw_extras when present.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Package filter: same ת.ז. may have classic + deluxe on one order — pick first unused match.
    IF v_candidate_count > 1
      ...
      ELSIF v_candidate_count > 1 THEN
        -- Provider named a package but no remaining EZGO row matches — flag mismatch, not duplicate.
        ...
        VALUES
          (v_run_id, v_provider_row.id, v_sole_easygo.id, v_provider_row.provider_id, 'package_mismatch', v_provider.match_mode,
           'חבילה שונה: ספק="' || v_provider_row.package_type || '" / EasyGo="' || COALESCE(v_sole_easygo.package_type, '') || '"');
        v_mismatch_count := v_mismatch_count + 1;
        CONTINUE;
      END IF;
    END IF;

    -- CouponNo filter (Nofshonit: provider מזהה לקוח was CouponNo → stored in raw_extras).
    IF v_candidate_count > 1
       AND v_provider_row.raw_extras IS NOT NULL
    ...
    ELSE
      ...
      VALUES
        (v_run_id, v_provider_row.id, NULL, v_provider_row.provider_id, 'duplicate_match', v_provider.match_mode,
         'נמצאו ' || v_candidate_count || ' שוברי EasyGo תואמים — לא ניתן לבחור יחיד (חסר וריאנט/CouponNo?) · IDs: ' ||

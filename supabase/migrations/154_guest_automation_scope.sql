-- 154_guest_automation_scope.sql
-- Tiered automation for municipal/group remark occupants: courtesy_only = Stage 4
-- (mid_stay) cron only; room_ready stays exempt in whatsapp-send. Existing
-- automation_muted=true rows backfill to scope=muted (no courtesy_only backfill).

ALTER TABLE public.guests
  ADD COLUMN IF NOT EXISTS automation_scope TEXT NOT NULL DEFAULT 'full'
    CHECK (automation_scope IN ('full', 'courtesy_only', 'muted'));

COMMENT ON COLUMN public.guests.automation_scope IS
  'full = full pipeline; courtesy_only = mid_stay courtesy check only (+ room_ready manual); muted = no cron pipeline.';

CREATE INDEX IF NOT EXISTS idx_guests_automation_scope
  ON public.guests (automation_scope)
  WHERE automation_scope <> 'full';

-- Preserve legacy rows — no retroactive courtesy_only promotion.
UPDATE public.guests
SET automation_scope = 'muted'
WHERE automation_muted = TRUE AND automation_scope = 'full';

-- Restrictiveness rank: higher = more blocking.
CREATE OR REPLACE FUNCTION public.merge_automation_scope(existing TEXT, incoming TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE GREATEST(
    CASE COALESCE(existing, 'full')
      WHEN 'muted' THEN 2 WHEN 'courtesy_only' THEN 1 ELSE 0 END,
    CASE COALESCE(incoming, 'full')
      WHEN 'muted' THEN 2 WHEN 'courtesy_only' THEN 1 ELSE 0 END
  )
    WHEN 2 THEN 'muted'
    WHEN 1 THEN 'courtesy_only'
    ELSE 'full'
  END;
$$;

-- sync_suite_arrivals: persist automation_scope; keep automation_muted in sync.

...
-- ================================================================
-- 9. WHATSAPP QUICK TEMPLATES
-- Quick-access message templates for the Broadcast module.
-- Editable via the UI; fetched ordered by sort_order.
-- ================================================================

CREATE TABLE IF NOT EXISTS public.whatsapp_quick_templates (
  id          UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  name        TEXT        NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_wqt_sort ON public.whatsapp_quick_templates (sort_order);
...
CREATE INDEX IF NOT EXISTS idx_guests_last_visit   ON public.guests (last_visit);
CREATE INDEX IF NOT EXISTS idx_guests_suite        ON public.guests (suite);
...
-- Managers and admins can manage campaigns
...
-- Seed: 5 default Dream Island templates
INSERT INTO public.whatsapp_quick_templates (name, icon, content, sort_order) VALUES
(
  'בילוי מים וספא', '🏊',
  'היי {guest_name}! הבילוי היומי בדרים איילנד הוא מסע של ניתוק. מחכים לכם 1,500 מ״ר של תענוגות מים, 3 בריכות זרמים, חמאם טורקי וטיפולי ואטסו. לשריון מקום: [לינק]',
  1
),
(
  'קולינריה - ארמונות ועמדות אוכל', '🍽️',
  'היי {guest_name}! מסעדת ארמונות פתוחה וממתינה לכם עם תפריט שף מיוחד. בנוסף — פוד טראקים ברחבי הריזורט כל היום. תיאבון! 🍷',
  2
),
(
  'סוויטות VIP', '👑',
  'היי {guest_name}! הסוויטה שלכם מוכנה ומחכה. בריכה פרטית, ג׳קוזי, וחצר מלכותית רק לכם. Check-in מ-15:00. מחכים לכם! 🏝️',
  3
),
(
  'חוויה יומי Premium Day', '🍷',
  'היי {guest_name}! חוויית ה-Premium Day שלנו כוללת קוטג׳ פרטי, קוקטיילים ופירות טריים + גישה מלאה לכל מתחם הספא. מושלם ליום כיף! לפרטים: [לינק]',
  4
),
(
  'נהלים', '👔',
  'היי {guest_name}! תזכורת — הריזורט פתוח א׳-ה׳ 09:00-21:00, שישי 09:00-18:00, שבת 09:00-19:00. הבריכות נסגרות שעה לפני. לכל שאלה — 08-6705600',
  5
)
ON CONFLICT DO NOTHING;

-- RLS: any authenticated user can read/write templates
ALTER TABLE public.whatsapp_quick_templates ENABLE ROW LEVEL SECURITY;
CREATE POLICY "wqt_all" ON public.whatsapp_quick_templates
  FOR ALL USING (auth.uid() IS NOT NULL);

-- Auto-update updated_at
CREATE TRIGGER trg_wqt_updated
  BEFORE UPDATE ON public.whatsapp_quick_templates
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
